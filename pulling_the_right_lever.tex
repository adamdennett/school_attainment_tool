...
Figure~\ref{fig-quadrant-overview} previews this four-quadrant view for
England's secondary schools. Each dot is one school. In the HTML version
of this paper the plot is interactive: hover over any dot to see the
school's name, local authority, region, Ofsted rating and the two
underlying values; switch between Ofsted-rating and region colourings
using the tabs; and use the dropdown filter to focus on a single local
authority's schools at a time. The dense cluster around the origin is
the typical case --- most schools nationally sit close to their
intake-predicted attainment with attendance close to what their intake
predicts. The interesting cases are the schools furthest from the

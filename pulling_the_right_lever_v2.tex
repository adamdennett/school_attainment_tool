...
\item
  ...
  by attendance alone.
\item
  Prior attainment at Key Stage 2 is the other dominant factor --- a
  large share of the variation in GCSE outcomes reflects accumulated
  inequalities from before secondary school.
\item
...
\item
  The local authority level Gorard Segregation Index reveals that
  Brighton and Hove is consistently in the top quarter of LEAs in
  England for the integration of disadvantaged and non-disadvantages
  students in its schools, however the Gorard Segregation variable is
  not statistically significant in the full model. Any harm that
  segregation causes is already fully mediated by the other variables in
  the model.
\item
...
\item
  ...
  ranking 15th for disadvantaged attainment, 4th for non-disadvantaged,
  and 5th overall in our new analysis --- once structural factors are
...
\item
  ...
  points - but crucially not for disadvantaged students (the intended
  beneficiaries of the council policy). The policy was pulling a lever
  with relatively little mechanical advantage in the wrong direction,
  while ignoring one with considerably more.
\item
...
\item
  \emph{For the Brighton and Hove City Council:} Recognise that the
  biggest factor affecting disadvantaged attainment in the city is
  absence from school, not social mixing - the city's schools are
  already some of the most socially mixed in England. Commission a study
  to understand what the different factors are that are leading to the
  city having the second-worst absence rate in England, understand the
  full range of impacts that absence has on different groups and develop
  policies to address these specific challenges while being fully
  mindful of negative externalities.
\end{itemize}

\textbf{What has changed in Version 2.0, and what it means}

Version 2.0 reworks the analysis in response to peer review of the
accompanying journal article, and to criticism the original report
attracted. The headline argument is unchanged, and in two respects it is
now stronger. But three numbers reported in Version 1 have moved, and
one of them moved against us, so it is worth being explicit about the
effect on the conclusions.

\begin{itemize}
\tightlist
\item
  \textbf{The core finding is unaffected.} Version 1 controlled for
  prior attainment using only the share of pupils arriving with
  \emph{low} KS2 scores. Version 2 uses the cohort's \emph{mean} KS2
  score, which captures the whole intake distribution. The absence
  coefficient moves by under 3\% --- so the central claim, that
  attendance is the most consequential lever available to Brighton and
  Hove, does not depend on the choice at all.
\item
  \textbf{The council's chosen lever looks weaker still.} Properly
  controlled, the association between concentrations of disadvantage and
  attainment falls by about a \emph{quarter}. Version 1's weaker control
  had left part of the intake difference between high- and low-FSM
  schools unmeasured, and the disadvantage variable was absorbing it.
  The case for redistribution as an attainment policy is correspondingly
  weaker than Version 1 implied.
\item
  \textbf{The most contested finding is now clearer, not weaker.} The
  positive association between disadvantage concentration and
  \emph{disadvantaged} pupils' attainment --- the result that caused
  most argument locally --- becomes substantially more clearly
  distinguishable from zero under the better control.
\item
  \textbf{One number moved against us.} Brighton and Hove's rank for
  disadvantaged attainment, after structural adjustment, falls from 7th
  of 151 authorities to 15th. The city remains within the strongest
  tenth, and the substantive point stands, but the earlier figure partly
  reflected the weaker control and we have corrected it throughout
  rather than quietly restating it.
\item
  \textbf{What absence actually costs the city is now quantified.} The
  city's standing depends almost entirely on whether absence is
  controlled for: 15th with that control, 69th without it. The gap
  between those two rankings is a measure of what the attendance problem
  costs the city's disadvantaged pupils, and it is worth roughly 1.4
  Attainment 8 points per pupil. This is new in Version 2.0, and it
  sharpens the policy conclusion considerably.
\item
  \textbf{Absence is not simply a symptom of poverty.} Version 2.0
  answers the argument that attendance cannot usefully be tackled
  directly because poverty will keep regenerating it. Across English
  local authorities, differences in poverty explain almost none of the
  difference in absence, and Brighton and Hove has the largest excess
  absence, relative to its disadvantage profile, of any authority in
  England.
\item
  \textbf{A note of realism about schools.} Version 2.0 sets the
  findings against sixty years of school-effectiveness research, which
  consistently puts the share of variation in pupil outcomes
  attributable to differences \emph{between} schools at roughly 8--15\%.
  Any policy that moves pupils between schools is working on the smaller
  part of the problem.
\end{itemize}
...
\begin{tcolorbox}[enhanced jigsaw, opacityback=0, breakable, leftrule=.75mm, arc=.35mm, bottomrule=.15mm, rightrule=.15mm, colframe=quarto-callout-note-color-frame, left=2mm, colback=white, toprule=.15mm]
\begin{minipage}[t]{5.5mm}
\textcolor{quarto-callout-note-color}{\faInfo}
\end{minipage}%
\begin{minipage}[t]{\textwidth - 5.5mm}

\vspace{-3mm}\textbf{What is new in Version 2.0}\vspace{3mm}

This report was first published in May 2025. Version 2.0 incorporates
the analysis carried out during peer review of the accompanying journal
article, together with responses to criticism the original report
attracted. The substantive additions are:

\begin{itemize}
\tightlist
\item
  \textbf{A better control for prior attainment.} Version 1 controlled
  only for the \emph{share of pupils arriving with low} KS2 attainment.
  Version 2 uses the cohort's \emph{mean} KS2 score, which captures the
  whole intake distribution rather than one tail. This is a materially
  better model, and it changes several of the numbers reported in
  Version 1 --- see Section~\ref{sec-prior-measure}.
\item
  \textbf{The cost of absence, quantified.} Brighton and Hove's standing
  for disadvantaged attainment depends almost entirely on whether
  absence is controlled for. The gap between those two rankings turns
  out to be a measure of what the city's attendance problem costs its
  disadvantaged pupils --- see Section~\ref{sec-cost-of-absence}.
\item
  \textbf{A response to the ``mould and damp'' argument}, that absence
  is merely a symptom of poverty and cannot usefully be tackled on its
  own (Section~\ref{sec-mould-and-damp}).
\item
  \textbf{How much schools can matter at all}, set against sixty years
  of school-effectiveness research
  (Section~\ref{sec-how-much-schools-matter}).
\item
  \textbf{A note on statistical inference} with population rather than
  sample data (Section~\ref{sec-inference}).
\end{itemize}

Where Version 2 revises a number reported in Version 1, it says so
explicitly rather than quietly restating it. Version 1 remains available
for reference.
...
This is a very broad backdrop which helps partially situate this report.
It emerges from a unique local situation in Brighton and Hove, England
--- a city not new to experimental schools policies (Allen et al., 2013)
and a local case study which helps highlight the very real dangers and
unintended outcomes of policy makers attempting to affect broad-brush
`solutions' but without a complete understanding of the problems in
their local system alongside its unique facets and challenges. Policies
which fail to fully understand the specific nature of the local
challenges, or pay heed to the negative externalities, will struggle to
prescribe an effective treatment and could do more harm than good.

We will argue in this report that while the evidence base is vast and
the data plentiful, in Brighton and Hove (and possibly therefore in
other LEAs) there can be a crucial disconnect between theory, evidence
and local policy making which must be addressed before considering
policy changes. While any local education authority might outwardly
claim to aspire to evidence-based or evidence-led policy, where good
evidence is not easily to hand, where there is not the internal capacity
within councils to synthesise theory and data at the pace prescribed by
the politicians, or where evidence is selectively filtered to fit a
predetermined policy objective, then the risk of making poor decisions
is high.

This report offers a different evidence and analytics-based approach to
this challenge through presenting both an analysis and an associated
tool which should fill many of the localised evidence voids that
currently face many LEAs looking to follow through on the national
government's objectives. Our assertion is that the Department for
Education already provides most of the raw materials required for making
better decisions through its open data publications, but these are
under-utilised. If this data can be made easily accessible and
digestible for those charged with improving the situation --- the
schools themselves, their leadership teams and governing boards, the
local education authorities or parents asked to contribute to statutory
consultations --- it will be more feasible to convert good intentions
into area-wide beneficial outcomes.
...
interesting case study as it objectively out-performs most other LEAs in
England but where new policy proposals in 2024 and 2025 reflected a poor
grasp of the available evidence. Council Policy decisions have
controversially opened the door to unintended negative social,
environmental, educational, and health outcomes while our analysis shows
the levers pulled are unlikely to lead to the stated educational policy
goals being met. Critically, the approach has led to a confused and
divided populace where parents might now be making sub-optimal choices
for their children while living in a knowledge vacuum. We hope that
through illustrating the particular issues in Brighton and Hove and
offering potential practical solutions, those in other local education
authorities might avoid similar mistakes. We note that the city is also
likely to be one of the places targeted by the DfE's new ``Mission
Coastal'' where issues with disadvantaged communities are relevant, so
an enhanced evidence-base will be timely.

Before examining the Brighton and Hove proposals specifically later in
the report, we wish to be explicit about the scope of our argument. Our
...
communities, which we do not dispute. Indeed it is worth acknowledging
that Brighton and Hove is one of the most integrated LEAs in England
already in terms of the distribution of disadvantaged students across
schools in the LEA - consistently in the top 1/3 of LEAs in England
across the 4-years of data we analysed. We are also aware that our
...
The lead author of this report is a resident of Brighton and Hove with
lived experience of the effects of the policy development process on
local families. This work has emerged from 18 months of discussions with
a wider community of parents in the city keen that their children are
not negatively impacted by erroneous policy decisions; some of these
parents are academics familiar with data and quantitative methods, some
are experienced in national policy making, some are involved in schools
in the city either through their governing boards or as teaching staff
and some are simply parents and residents who want to see good education
policy making that will benefit all children in the city. As such, the
report builds on a diverse range of perspectives which are relevant to
the national government's ambitions.

This paper will proceed with a review of the recent literature on
...
England is plentiful. Most relates to attainment at the end of National
Curriculum Key Stage 4 (KS4) measured either in General Certificate of
Secondary Education (GSCE - the exam most take at age 16) points,
particularly in subjects such as Maths or English, or Attainment 8 ---
an aggregation of GCSE points across multiple subjects. The exact
metrics relating to GSCEs can vary across studies. For example, Morris
et al. (2021) measure fine graded point scores, while Houtepen et al.
(2020) use both continuous GCSE points based on top 8 qualifications and
a binary threshold of 5+ GCSEs at grade C or above including English and
Maths. One explanation for this variation --- and indeed, complication
--- is that the way in which GCSEs are graded has evolved over time, as
has what people have conceived as the `gold standard'. Gillborn et al.
(2017) track the evolution from five or more A*-C grades in any subjects
(introduced 1988) to the `gold standard' requiring English and
mathematics (2005), and subsequently to the English Baccalaureate
including sciences, humanities, and languages, and the issues this has
caused for defining the `attainment gap'. In another piece, Odell (2017)
focus on the percentage of students achieving grades C or higher in
English, mathematics and at least three other subjects.
...
example, exam grades at National Curriculum Key Stage 2 (KS2 - the
latter stages of Primary School from ages 7 to 11\footnote{Details of
  English National Curriculum Key Stages
  \url{https://www.gov.uk/national-curriculum}}) entry, with grades at
KS4 exit. Gorard and Siddiqui (2019) compare KS4 outcomes with prior
attainment at ages 6, 10, and 13. Gorard et al. (2025) specifically
analyse ``Progress 8'' scores which measure progress between KS2 and KS4
while Stopforth and Gayle (2025) examine the relationship between
results at KS2 and subsequent GCSE outcomes. Progress 8 adds a little
more nuance to the attainment picture, but it has also been criticised
(Leckie and Goldstein, 2019; Prior et al., 2021) mainly because it fails
to adjust for pupil background characteristics which are relevant in
predicting progress, particularly in places like London. In our
analysis, we choose to focus on Attainment 8 and use prior attainment as
one of the predictors, rather than incorporate it into the outcome. As
we shall see, prior attainment has important interactions with other
variables that if ignored, can give a false impression of causality
which is important to account for. In addition, Attainment 8 is cited as
the measure of choice for assessing the attainment gap in the recent DfE
...
It is also worth mentioning that while Gorard and Siddiqui (2019)'s
pupil-level analysis suggests a negative impact of disadvantaged
clustering, it runs counter to a 2015 Department for Education report on
the drivers of disadvantaged attainment (Macleod et al., 2015) which
reports that higher concentrations of disadvantage in schools are
associated with increased success for disadvantaged pupils at KS4.
...
\subsubsection{Journey to School}\label{journey-to-school}

If attendance sits at the heart of the attainment story, then anything
that systematically erodes attendance deserves attention in its own
right. One factor that the wider literature highlights, but which rarely
surfaces in English attainment analyses, is the daily journey to school.
The journey-to-school literature is not strictly about attainment ---
its outcomes are typically measured in sleep, health, wellbeing or
absence rather than GCSE points --- but its findings matter for
attainment by virtue of the chain of mediation that runs through
attendance. This becomes a particularly relevant strand of the
literature in the policy context discussed later in this paper, where
admissions reforms premised on social mixing necessarily entail longer
or more complex daily commutes for some pupils.

The most direct link between travel and attainment runs through absence.
Thomson (2023), using English National Pupil Database data, shows that
pupils with longer journeys to school have measurably higher rates of
absence, with effects concentrated among already-disadvantaged groups.
Similar patterns have been documented internationally: Cordes et al.
(2022) use a natural experiment in New York City school-bus disruptions
to show that transit problems directly produce absence; Blagg et al.
(2018), in a Washington DC study, find that students who travel further
from home to school are absent more often and switch schools more
frequently; and Otsuka et al. (2025) report comparable commute-absence
relationships in a Japanese setting. The mechanism is intuitive ---
longer or more uncertain journeys leave less margin for the small
frictions (illness, transport delay, weather, family logistics) that
turn a marginal day into an absent one --- but it is also empirically
robust across very different national contexts.

Beyond absence, a substantial body of work documents the broader costs
that long commutes impose on children. Long journeys are repeatedly
associated with reduced sleep duration and poorer sleep quality, with
knock-on consequences for psychosocial functioning (Fredriksen et al.,
2004; Yeo et al., 2019). Active and short commutes, conversely, are
linked to better physical health and fitness outcomes (Faulkner et al.,
2013; Pereira et al., 2014; Pradhan and Sinha, 2017; Voulgaris et al.,
2019) --- a literature that also makes the converse point that
displacing pupils into longer car or bus journeys removes a meaningful
slice of daily physical activity. Mental health and stress consequences
have been documented in adolescent and student populations across
multiple settings (Chairassamee et al., 2024; Guan et al., 2025; Karan
et al., 2021; Nakajima et al., 2024), with broader wellbeing effects
(Ding et al., 2023). None of these studies treats GCSE attainment as the
outcome, but each describes a pathway --- disrupted sleep, poorer
physical health, elevated stress, lower wellbeing --- that the
attendance and individual-factors literatures already identify as
upstream of school performance.

There is also a more qualitative dimension to displacing pupils from
their local schools that the attainment-focused literature tends to
under-weight. Schools function as community anchors: walked-to schools
support after-school participation, friendship groups that map onto
local catchments, parental engagement that depends on geographic
proximity, and the active-travel pathways that support physical and
mental health (Faulkner et al., 2013). Where redistribution policies
extend daily commutes, they tend to do so unevenly. Shah (2023), in a
case study of extended commutes in Chicago, argues that the burden of
long school journeys falls disproportionately on pupils already
experiencing other forms of marginalisation. Stein et al. (2021), using
US transit data, shows that long-distance transit predicts early high
school transfer --- itself a known risk factor for poor attainment.
These are not peripheral concerns: they are part of the cost-benefit
calculus that any policy reorganising pupil flows should engage with,
and they form a strand of evidence that we will return to when assessing
the Brighton and Hove proposals.
...
\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-catchments-1.pdf}}
...
with the summary of the feedback from the council stating:

\begin{quote}
\emph{``While there was broad agreement about the importance of
maintaining thriving schools and ensuring educational equity,
significant concerns were raised about the proposed methods for
achieving these goals. The feedback revealed a strong preference for
...
The council's December 2024 cabinet papers\footnote{Brighton and Hove
...
set out three stated objectives for the proposals: \emph{``a school
system where all pupils get access to a great education''}, improving
\emph{``the education offer for disadvantaged pupils by reducing some
schools' barriers to success''}, and using \emph{``all available options
to maintain a broad curriculum and holistic school experience for all
pupils''}. These were to be delivered alongside the council's broader
School Organisation Strategy commitment to \emph{``schools which are
sustainable and able to thrive''} against a backdrop of falling pupil
numbers across the city. The intended design principles included
\emph{``better equality of outcomes --- results not driven by economic
advantage''} and a \emph{``comprehensive offer from our city schools as
a more mixed pupil intake creates better outcomes for disadvantaged
pupils''}, while \emph{``maintaining the geographic spread of secondary
schools in the city''}. The final proposals in the consultation saw a
key catchment boundary change in the east of the city and reductions in
overall places available at two of the most popular and oversubscribed
schools (Blatchington Mill and Dorothy Stringer). Fewer places would be
available for children living in central catchments as 20\% of the
remaining places (after PAN reductions) would be reserved for out of
catchment children ahead of in-catchment children. The outcome of these
combined policy decisions (effectively reducing available places for
in-catchment children within the Varndean and Dorothy Stinger catchment
by 150) would be to force children (beginning from 11 years of age) from
the central catchments to attend under-subscribed schools at the
periphery of the city, potentially requiring over 2 hours of commuting
on public transport per day if those children were allocated the
furthest and most under-subscribed school. While useful for the council
balancing budgets, this was not popular with those parents whose
children faced being unable to attend their local school.
...
the first place. We should highlight that following widespread
opposition and appeals to the schools adjudicator following the 2024
consultation, the proposed PAN reductions at central schools were
rejected. And under pressure from a negative public response the 20\%
out-of-catchment priority was reduced to 5\%. This meant that following
national offer day in March 2026, nearly all in-catchment pupils who
wanted to, were able to attend their catchment school. However, while
displacement was minimised, the full 5\% out-of-catchment quota was
taken up by opt-in students and significant numbers of pupils were
opting to travel between schools in the outer catchments where
under-subscription allowed unhindered movement. And it is also the case
that while the most radical changes did not come to pass following the
2024 consultation, the belief in the efficacy of the policy within the
council appears to have persisted. A subsequent 2025 consultation once
again sought to increase the out-of-catchment quota beyond 5\% via
placing siblings of students allocated an out-of-catchment school, ahead
of in-catchment children in the priority list, and not counting them
towards the 5\%. As such, it is worth scrutinising the original policy
and evidence-base further.

From the papers accompanying proposals put forward, as well as the
content of the council's People Overview and Scrutiny Committee meeting
held on 9th October 2024, it is clear that the council had settled on an
attempt to affect increased mixing of disadvantaged and
non-disadvantaged students in its secondary schools in the belief that
this would raise disadvantaged attainment. What it didn't acknowledge in
those papers was that across the schools in Brighton and Hove, the
mixing of disadvantaged and non-disadvantaged students was already high.
Using one measure of segregation, the \emph{Gorard Segretation Index,}
social mixing in Brighton and Hove's schools places it in the least
segregated quarter of LEAs, by disadvantage \footnote{\url{https://adamdennett.github.io/school_attainment_tool/brighton_case_study.html\#sec-gorard-context}}).
And it was not articulated how much additional benefit attempts to
engender still further mixing would bring, either in absolute terms or
relative to any disbenefits.

Underpinning the case was a reading of Gorard and Siddiqui (2019) (who
also appeared via video link in the scrutiny committee meeting) in which
more mixed intakes produce better outcomes for disadvantaged pupils,
plus a premise that attainment in the city was \emph{`driven by economic
advantage'}. The proposals put out for consultation at the end of 2024
were in addition to a policy change the previous year that gave students
eligible for Free School Meals priority over most other students (both
in- and out-of-catchment) in the event of a tie-break over available
places --- a policy which went through without objection in the city and
one which was already working to affect further social mixing in
schools. By reducing the numbers of places available at popular schools
in the centre of the city --- combined with an out-of-catchment priority
for a number of students --- there would be a number of pupils living in
the centre of the city without a place in a local school for them to
attend. As such the policy had another, but poorly articulated benefit
for the local authority: without a local school place, if parents agreed
to send their children on a commute to under-subscribed schools on the
geographic periphery of the city where places remained, it would support
these schools to remain financially viable and help mitigate the
increased depopulation of those catchments exacerbated by the
...
which might be even more important. The challenge for policy makers and
consultation responders alike is in how we disentangle these factors and
judge their relative importance, not just in general terms, but in the
terms that relate to the very specific local geographic contexts that
present themselves in different Local Education Authorities. Is there
evidence that is easy to hand? Evidence in addition to that tucked away
in the National Pupil Database --- an admittedly fantastic resource, but
one which no-one can access rapidly --- and which schools and parents
under pressure to respond rapidly to policy proposals can make use of?
...
\hspace{1em}ks2\_c & 12199 & 99.9 & 4.31 & 2.79 & -8.30 & 4.10 & 17.40\\
...
\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-bivariate-raw-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-bivariate-log-1.pdf}}
...
\subsection{A note on statistical inference}\label{sec-inference}

A word is warranted on what the significance values in the tables that
follow do and do not mean, because the data here are unusual.

Our panel is not a random sample drawn from some larger population of
schools. It \emph{is} the population: every state-funded mainstream
secondary school in England meeting our inclusion criteria. We therefore
do not interpret standard errors as measuring uncertainty about
generalising from a sample to a whole. We interpret them as measuring
the stochastic, cohort-to-cohort variation that produces any given
school's result in any given year. Each year group is one realisation of
a process that would have turned out somewhat differently with a
different set of children, and school-level scores are correspondingly
unstable from year to year --- markedly so in smaller schools. The
multilevel structure is a further guard against over-reading that
instability, since partial pooling shrinks the noisiest estimates toward
the relevant group mean.

Consistent with long-standing cautions about the over-interpretation of
p-values (Wasserstein and Lazar, 2016), we treat statistical
significance here as descriptive rather than decisive. No conclusion in
this report turns on a threshold being crossed. The arguments are made
in Attainment 8 points, they are reproduced across four separate years,
and they would stand unchanged if every test statistic were stripped
out.
...
Admissions: Other non-selective (ref: non-sel. in highly sel. area) &  &  &  &  &  &  & 0.039*** & 8.73 & 0.028*** & 4.06\\
Admissions: Selective (ref: non-sel. in highly sel. area) &  &  &  &  &  &  & 0.019* & 2.44 & -0.004 & -0.55\\
...
Random effects &  &  &  &  &  &  &  &  &  & \\
LA (nested in region) &  &  &  &  &  &  &  &  & 0.026 & (SD)\\
Region &  &  &  &  &  &  &  &  & 0.014 & (SD)\\
Ofsted rating &  &  &  &  &  &  &  &  & 0.045 & (SD)\\
Year &  &  &  &  &  &  &  &  & 0.045 & (SD)\\
Residual &  &  &  &  &  &  &  &  & 0.092 & (SD)\\
 &  &  &  &  &  &  &  &  &  & \\
R² & 0.3932 &  & 0.6117 &  & 0.6961 &  & 0.7093 &  & 0.6512 & (marginal)\\
R² (conditional) &  &  &  &  &  &  &  &  & 0.7793 & (fixed + random)\\
...
Figure~\ref{fig-bivariate-log}. Models 2 and 3 then add absence and mean
KS2 prior attainment with the changes in the coefficients showing first
the mediating effect of absence and then the additional mediating impact
of prior attainment while both push the explanatory power of the model
up significantly. Models 4 and 5 add additional fixed and then finally
random effects into the explanatory mix.
...
coefficients shrink again (FSM goes from -0.122 to -0.047, absence from
-0.364 to -0.259). There could be an argument that prior attainment
confounds some of the effects of absence on attainment (in some cases,
it could be a common cause of both), however both prior attainment and
absence act as mediators sitting between deprivation and attainment
...
close to 70\% of the variation in attainment across schools in England.
...
measure). With t = -60.5, absence is doing most of the heavy-lifting.
Mean KS2 prior attainment (t = 58.2) is close behind, confirming the
idea that a large amount of the variation in attainment at age 16 is
explained by the levels of attainment pupils arrive at secondary school
with. Note that this coefficient is \emph{positive}: cohorts arriving
from primary school with higher average KS2 scores go on to higher
Attainment 8. In this three variable model, there is still a residual
poverty effect, but it is now much the smallest of the three. The
proportion of deprived pupils in a school (t = -21.0) that remains after
we account for absence and prior attainment is still statistically
significant, suggesting that deprivation is an ongoing hindrance for
attainment --- but its influence is roughly a third of that of absence.
However, there might be additional variables which add to the story and
...
of these suppress the effects of the deprivation, prior attainment and
absence triad, reflecting to a degree a school-level institutional
...
power to the model with a shift in R-squared from 0.65 (Marginal) to
0.78 (Conditional) indicating that 13\% of the variance in school-level
...
\subsubsection{Why we changed how prior attainment is
measured}\label{sec-prior-measure}

Version 1 of this report controlled for prior attainment using the
\emph{percentage of pupils arriving with low KS2 attainment}. Version 2
uses the cohort's \emph{mean} KS2 scaled score instead. The change was
prompted by a reviewer of the journal article, and it turned out to
matter more than we expected.

The problem with the low-attainment share is that it sees only one end
of the intake distribution. It records how many pupils arrived already
behind, but says nothing about how many arrived ahead. Two schools with
identical proportions of low prior attainers can have very different
intakes across the rest of the range, and a model that cannot tell them
apart will systematically under-adjust for schools whose advantage sits
at the \emph{top}. Mean KS2 reflects both tails, and fits substantially
better: it improves model fit by a wide margin, and once it is included
the low-attainment share adds nothing at all.

Three consequences are worth reporting plainly, because two of them
revise figures given in Version 1.

\textbf{The absence finding is unaffected.} The absence coefficient
moves by under 3\% in either pupil group. The central argument of this
report does not depend on the choice.

\textbf{Concentrations of disadvantage matter even less than Version 1
suggested.} The coefficient on disadvantage concentration falls by
roughly a \emph{quarter} for all pupils. The low-attainment share had
been leaving part of the intake difference between high- and low-FSM
schools unmeasured, and the FSM variable had been quietly absorbing it.
Properly controlled, the lever the council chose is weaker still.

\textbf{The grammar-school effect disappears entirely.} This is the most
striking consequence. Under Version 1's control, selective schools
appeared to add a large and highly significant premium to attainment.
Under mean KS2 that premium vanishes --- the coefficient is
indistinguishable from zero. The reason is visible in the intake data:
selective schools average a KS2 score of about 113 against 104
elsewhere, but their share of \emph{low} prior attainers is 0.3\%. That
measure is pinned at the floor and simply cannot express how selective a
grammar school's intake is, so the unmeasured advantage re-emerged as an
apparent school effect. Once intake is measured properly, selection
explains none of the difference in attainment. This is exactly what the
wider literature has long concluded (Burgess et al., 2018), and Version
1 was, in effect, quietly contradicting it.

\subsubsection{What the disadvantage coefficient does and does not
measure}\label{sec-fsm-caution}

One interpretive caution follows from all this, and it cuts against a
reading our own results might otherwise invite.

Prior attainment at Key Stage 2 is not independent of disadvantage. It
is, in substantial part, its accumulated product: the poverty a child
experiences before secondary school shapes the score they arrive with
(Stopforth and Gayle, 2025). A control that captures the whole intake
distribution therefore absorbs correspondingly more of the poverty
signal --- which is precisely why the disadvantage coefficient shrinks
when we improve the control.

The coefficient that survives should not, then, be read as an estimate
of how much poverty matters to attainment. It is a \emph{within-intake}
comparison: what remains once schools receiving similar cohorts are set
alongside one another. The total contribution of disadvantage to
attainment is substantially larger than that residual coefficient
implies, and nothing here should be taken to suggest otherwise. What
these models can identify is the marginal association with the
\emph{concentration} of disadvantage at school level, holding intake
constant --- a much narrower quantity, and the one the 2024 policy
proposition actually rested on.
...
\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-standardised-coefficients-1.pdf}}
...
on attendance and on mitigating the impacts of weak prior attainment
should be the priorities for policy makers. Any attempts to affect
attainment through changing the proportions of disadvantaged and
non-disadvantaged students in schools are unlikely to have much impact
at the school-level and could have the opposite impact on disadvantaged
students. But beyond this what is really required for policy makers,
school leaders and parents alike, is an ability to translate these
general observations to very specific local contexts --- contexts that
really matter where schools will sit along a continuum for each of the
variables that might impact attainment and where many of the
associations with attainment are non-linear meaning that where along the
continuum really matters for the size of the effect --- whether this is
changes to attendance, concentrations of disadvantage or staff
retention.

\subsection{Decomposing absence: structural intake versus school
management}\label{sec-decomposing-absence}

The headline result above places absence as the single most important
predictor of school-level Attainment 8. But this finding raises a
complication that becomes important when we move from national patterns
to specific schools and local authorities. Absence as it appears in our
model is not a single causal entity. It sits in two roles at once.

Some of school-level absence is \textbf{exogenous to the school}: driven
by family circumstances, area health, deprivation patterns, the SEN
profile of the catchment, and other features of the intake the school
inherits but cannot directly choose. Some of it is \textbf{produced by
the school}: through pastoral systems, follow-up on persistent
absentees, parental engagement, attendance officers, and the broader
management ethos around getting children into the building.

When we control for raw absence in our headline model, both components
are absorbed together. The school-level residual that remains --- what
we have been calling its ``value-added'' --- reflects what the school
does \emph{given} its attendance, not what it does \emph{to} its
attendance. A school that has worked hard to lift attendance for an
intake that would otherwise carry high absence rates gets no specific
credit for that work; it shows up in the absence variable and is removed
from the residual. The reverse is true too: a school that runs higher
absence than even its already-disadvantaged intake would predict cannot
easily be distinguished from one whose absence is wholly structural.

In supplementary analyses\footnote{Full technical details, model fits
  and diagnostics:
  \url{https://adamdennett.github.io/school_attainment_tool/model_experiments.html\#sec-two-stage-absence}},
we address this with two parallel models built on a single principle:
adjust for what a school inherits, not for what it does. The
\textbf{first model} predicts school-level absence from the intake
variables we treat as exogenous (FSM, EAL, mean KS2 prior attainment,
segregation, plus place and year random effects), yielding two
quantities for every school-year: an \emph{expected absence} (the part
the intake predicts) and a \emph{residual absence} (what is left over
--- broadly, the school-controllable component). The \textbf{second
model} predicts attainment from those same intake variables, with no
absence control and no workforce predictors, since attendance management
and workforce decisions are things a school does rather than things it
receives. The school-level residual from this second model is a more
honest value-added measure: it gives credit for what the school adds to
attainment over and above its intake, \emph{with} attendance management
already counted as part of the school's contribution rather than
stripped away as a control.

Two caveats follow from this design. Because neither model controls
observed absence, absence that is genuinely outside a school's control
but not captured by intake or place will be charged to that school's
value added --- the first model explains 55\% of absence variation, so
the remainder is not trivial. And the second model's coefficients cannot
be read as effects: with absence left out, the disadvantage term absorbs
it and roughly doubles in size. We therefore use its residuals only, and
every disadvantage estimate quoted elsewhere in this report comes from
the headline model, which controls absence directly.

The national-level picture this paints is informative. A little over
half of the school-year variation in absence is explained by the first
model --- it is dominated by intake and place-level structural factors.
The school-controllable residual is the smaller share, and at population
level its marginal contribution to attainment is modest after
intake-driven absence is already accounted for. This does not mean
attendance management is unimportant --- it means \emph{the headline
raw-absence coefficient borrows much of its statistical force from the
structural component}. The lever is real, but the school-only portion of
it is smaller than the single coefficient implies. For local education
authorities, the implication is clear: closing a city's absence problem
will require both school-level attention \emph{and} cross-departmental
work --- public health, children's services, area-level deprivation
policy --- because much of what schools are dealing with on attendance
is inherited rather than generated.

For local school-level diagnostics, however, the decomposition becomes
considerably more informative. Two signals can now be read side by side:
the school's total value added, and its residual absence (positive when
the school runs more absence than its intake would predict, negative
when less). Sorted on these two axes, schools fall into four meaningful
policy stories. Schools that add value \emph{and} manage attendance
better than their intake predicts present an internally consistent
strong-management profile. Schools that add value \emph{despite}
worse-than-predicted attendance are pedagogically impressive cases where
the obvious next lever is attendance. Schools that underperform
\emph{and} run worse-than-predicted absence are the cleanest
single-lever targets --- both signals point the same way. Schools that
underperform \emph{despite} unusually good attendance management are the
most awkward cases for an LEA to act on, since the gap must come from
somewhere other than absence and the levers available are all harder to
pull.

The figure below previews this four-quadrant view for England's
secondary schools. Each dot is one school. In the HTML version of this
paper the plot is interactive: hover over any dot to see the school's
name, local authority, region, Ofsted rating and the two underlying
values; switch between Ofsted-rating and region colourings using the
tabs; and use the dropdown filter to focus on one or more local
authorities at a time (clear the selection to return to all schools).
The dense cluster around the origin is the typical case --- most schools
nationally sit close to their intake-predicted attainment with
attendance close to what their intake predicts. The interesting cases
are the schools furthest from the origin, in any direction.

\begin{figure}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-quadrant-overview-1.pdf}}

}

\caption{\label{fig-quadrant-overview}All English secondary schools on
the joint-signal plane, coloured by their most recent Ofsted rating.
Vertical axis: school-level value-added (Attainment 8 points).
Horizontal axis: the school-controllable absence component (percentage
points). The reference lines at zero on each axis split the plane into
the four narrative quadrants discussed in the text. (The HTML version of
this paper renders this as an interactive plot with a local-authority
filter and a region-coloured alternative view.)}

\end{figure}%

A small but striking pattern is already visible. Brighton and Hove's
schools cluster noticeably to the right of the national distribution on
the horizontal axis --- a city-wide tendency to run higher absence than
even the city's intake mix would predict. On the vertical axis the city
is more mixed, with several schools sitting comfortably above the
national centre of mass and a handful below it. We return to the
labelled, school-by-school version of this plot in the case study below,
where the named identity of each dot adds the local context this
overview cannot.
...
the model. Towards the right of the plot, 15th best out of 151 LEAs in
...
\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-caterpillar-1.pdf}}
...
city, we have a Local Education Authority that, given the structural and
school factors we know affect attainment, is over-performing in the
national context. The random intercepts in the caterpillar plot could be
viewed as an alternative LEA league table that places Brighton and Hove
in the top 10 LEAs for both disadvantaged and non-disadvantaged
attainment. Rather than asking what the city is doing wrong, we might
instead be asking what it is doing right? What are the factors (both
within the gift of the LEA to influence and not) not captured by the
model that mean that the city is over-performing? Are they home-based,
such as parental engagement? Is it something to do with school
leadership and governance? Local Authority services? School improvement
programmes? These questions will require additional data or qualitative
follow-up to find the answers to.
...
impressive? Why is the random intercept for Brighton not as positive and
large as it is for, say, Plymouth (another comparable South Coast city)
or larger cities like Manchester and Birmingham? What might be the most
effective policy levers to pull to improve overall disadvantaged
attainment even further in Brighton and Hove? And how do the most
effective levers compare with those that were chosen?

\subsection{The cost of absence}\label{sec-cost-of-absence}

The local-authority effect above is a \emph{conditional} result: it says
how Brighton and Hove's schools perform once absence, disadvantage,
prior attainment and the rest are accounted for. That conditioning does
a great deal of work in a city with the second-worst attendance in
England, and it is worth being explicit about how much.

Fitting the model with and without a control for absence, on an
identical sample, gives two very different pictures of the city.
\textbf{Controlling for absence, Brighton and Hove ranks 15th of 151
local authorities for disadvantaged attainment. Without that control, it
ranks 69th and is statistically indistinguishable from the national
average.}

Neither number is the whole truth, and it would be easy to reach for
whichever suits. Taken together, though, they say something more useful
than either alone: the city's schools appear to be genuinely effective
for the pupils who actually turn up, and the city's attendance problem
is large enough to erase that advantage entirely in the aggregate.

The distance between the two rankings has a concrete interpretation. It
is worth roughly \textbf{1.4 Attainment 8 points per disadvantaged
pupil} --- the difference between what the city's disadvantaged children
currently achieve and what they might achieve if their schools'
underlying effectiveness were not being drained away by absence. Read
this way, attendance is not a caveat to a good-news story about Brighton
and Hove. It is the story.

This also reframes what the 2024 consultation got right and what it got
wrong. The council was not wrong that the city has a problem:
unconditionally, its disadvantaged pupils do no better than the national
average, and the raw attainment gap that motivated the consultation is
real. What the consultation got wrong was the diagnosis. Presented with
an attainment gap, it reached for the social composition of schools ---
one of the weakest levers in our modelling, and for disadvantaged pupils
a lever that may point the wrong way --- while the factor that actually
separates the city's conditional standing from its unconditional
standing went undiscussed. The more accurate account is not that a
high-performing city was mistaken for a failing one, but that a city
with one specific, severe and addressable problem was prescribed a
treatment for a different problem it did not have.
...
\textbf{Revised from Version 1.} Version 1 of this report put Brighton
and Hove 7th of 151 for disadvantaged attainment. Under the improved
prior-attainment control described in Section~\ref{sec-prior-measure},
the city ranks 15th (and 69th without an absence control). The city
remains within the strongest tenth of authorities and the argument is
unchanged --- if anything the gap between the conditional and
unconditional rankings is slightly wider --- but the earlier figure
partly reflected the weaker control.
...
\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-accelerating-returns-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-accelerating-returns-standardised-1.pdf}}
...
\textbf{Brighton and Hove} & \textbf{10.8\% (144/152)} & \textbf{10.5\% (137/152)} & \textbf{11.0\% (143/152)} & \textbf{10.8\% (151/152)}\\
...
\midrule
Longhill High School & 16.7 & 8.2 & 8.4 & 15.76 & 5.2\\
Brighton Aldridge Community Academy & 14.4 & 8.2 & 6.2 & 12.37 & 4.5\\
Hove Park School and Sixth Form Centre & 12.9 & 8.2 & 4.6 & 9.72 & 4.0\\
Portslade Aldridge Community Academy & 11.3 & 8.2 & 3.1 & 6.87 & 2.9\\
Blatchington Mill School & 10.8 & 8.2 & 2.6 & 5.76 & 2.9\\
Dorothy Stringer School & 9.8 & 8.2 & 1.5 & 3.60 & 1.9\\
Varndean School & 9.0 & 8.2 & 0.8 & 1.93 & 0.9\\
Cardinal Newman Catholic School & 8.5 & 8.2 & 0.3 & 0.62 & 0.3\\
\bottomrule
\end{tabular}}
\endgroup{}
...
The council has recently agreed to the creation of a cross-party working
...
There may also be counter-intuitive interactions with absence and
learning that are smoothed out at the school-level and that can only be
unpicked at the classroom level and over time. For example, if otherwise
disruptive students switch to being absent students at a higher rate in
Brighton and Hove, this may have a positive impact on learning for those
who remain in school with smaller class sizes and teachers able to
devote more time to good pedagogy and less to classroom management. The
city really needs to disaggregate this attendance problem and its
impacts, and devise strategies to deal with the different causes and
...
\midrule
Brighton Aldridge Community Academy & 52.6 & 28.8 & -0.603 & 3.05 & 1.4\\
Longhill High School & 40.1 & 28.8 & -0.332 & 1.67 & 0.8\\
Hove Park School and Sixth Form Centre & 35.8 & 28.8 & -0.219 & 1.09 & 0.5\\
Portslade Aldridge Community Academy & 29.7 & 28.8 & -0.032 & 0.16 & 0.1\\
\bottomrule
\end{tabular}}
\endgroup{}
...
\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-prior-att-1.pdf}}
...
\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-obs-vs-pred-1.pdf}}
...
\caption{\label{tbl-league-all}All Pupils: value-added rankings (panel
model residuals by year).}
...
\midrule
1 & Dorothy Stringer School & +2.5 & +3.1 & +2.2 & +4.8 & \textbf{+3.1}\\
2 & Varndean School & +2.7 & +2.6 & +5.3 & +0.5 & \textbf{+2.8}\\
3 & King's School & +3.5 & +0.9 & +5.0 & -0.5 & \textbf{+2.2}\\
4 & Brighton Aldridge Community Academy & +2.0 & +1.4 & +1.2 & +0.8 & \textbf{+1.4}\\
5 & Portslade Aldridge Community Academy & +3.8 & +0.6 & +2.3 & -1.3 & \textbf{+1.4}\\
6 & Hove Park School and Sixth Form Centre & +1.5 & +0.9 & +1.4 & +0.4 & \textbf{+1.0}\\
7 & Blatchington Mill School & -1.5 & -1.0 & +2.0 & +2.4 & \textbf{+0.5}\\
8 & Cardinal Newman Catholic School & +1.3 & -2.2 & +1.3 & -1.5 & \textbf{-0.3}\\
9 & Longhill High School & -0.2 & -2.3 & -1.1 & -2.5 & \textbf{-1.5}\\
10 & Patcham High School & -2.7 & -2.8 & -4.4 & -2.9 & \textbf{-3.2}\\
\bottomrule
\end{tabular}}
\endgroup{}
...
\caption{\label{tbl-league-dis}Disadvantaged Pupils: value-added
rankings (panel model residuals by year).}
...
Rank & School & 2021-22 & 2022-23 & 2023-24 & 2024-25 & Mean Residual\\
\midrule
1 & Brighton Aldridge Community Academy & +3.1 & -1.3 & +4.6 & +5.7 & \textbf{+3.1}\\
2 & Varndean School & +2.2 & +3.2 & +3.9 & -2.4 & \textbf{+1.7}\\
3 & Cardinal Newman Catholic School & +2.9 & -5.4 & +6.5 & +2.3 & \textbf{+1.6}\\
4 & Dorothy Stringer School & +2.2 & +2.6 & -0.2 & +1.4 & \textbf{+1.5}\\
5 & Blatchington Mill School & -5.0 & +1.1 & +4.7 & +2.9 & \textbf{+0.9}\\
6 & King's School & +4.8 & +1.4 & +3.8 & -6.2 & \textbf{+0.9}\\
7 & Hove Park School and Sixth Form Centre & +3.0 & -2.5 & +2.0 & +1.1 & \textbf{+0.9}\\
8 & Portslade Aldridge Community Academy & +3.2 & -3.4 & +4.1 & -1.9 & \textbf{+0.5}\\
9 & Patcham High School & +4.1 & +1.5 & -11.6 & -3.5 & \textbf{-2.4}\\
10 & Longhill High School & -5.1 & -6.4 & +1.4 & -0.9 & \textbf{-2.7}\\
\bottomrule
\end{tabular}}
\endgroup{}
...
\subsection{The joint-signal view applied
locally}\label{the-joint-signal-view-applied-locally}

The decomposition introduced at the end of section 5 lets us look at
Brighton and Hove's secondary schools through a sharper lens. Each
school can now be placed on a two-axis plane
(Figure~\ref{fig-bh-quadrant}): vertically by its Expected Absence
value-added in Attainment 8 points (positive = adding value above intake
expectations, with attendance management already credited as part of
that contribution), and horizontally by its residual absence in
percentage points (positive = running more absence than intake predicts,
negative = running less). The four quadrants correspond to the four
school stories described earlier: schools adding value with attendance
helping (top-left), schools adding value despite worse-than-predicted
attendance (top-right), schools underperforming with attendance the
cleanest single lever (bottom-right), and schools underperforming
despite unusually good attendance management (bottom-left).

\begin{figure}

\centering{

\pandocbounded{\includegraphics[keepaspectratio]{pulling_the_right_lever_v2_files/figure-pdf/fig-bh-quadrant-1.pdf}}

}

\caption{\label{fig-bh-quadrant}Brighton and Hove secondaries on the
joint-signal plane: school value-added under M2 (vertical) versus the
school-controllable absence component (horizontal). Each point is one
school; reference lines at zero on each axis split the plane into the
four narrative quadrants discussed in the text.}
...
2024 admissions consultation framed Brighton and Hove's attainment story
as one of disadvantage concentration, and the proposed remedy was
redistribution. Both the headline modelling and this finer-grained
joint-signal view tell a different story: the city's attainment problem
is overwhelmingly an absence problem, but \textbf{the
school-controllable component of that absence is the smaller share ---
much of what individual schools are dealing with is structural,
inherited from intake and area context}. Closing the city's absence
problem requires both school-level attention (clearest in the
bottom-right quadrant) \emph{and} cross-departmental work the council
cannot delegate to schools alone. The attainment gains that would follow
are real and substantial, and dwarf anything achievable by changes to
...
analysis for the DfE, our more up-to-date modelling confirms these DfE
findings of a decade ago and that this was, at best, a flawed
understanding; at worst a belief driving policy with many losers and few
clear winners.
...
\subsubsection{Absence as the Primary Policy
Lever}\label{absence-as-the-primary-policy-lever}

It is worth pausing on this point in the context of the council's own
...
three core objectives for the 2026/27 admissions changes \footnote{Brighton
  ...
  (determination):
  \url{https://democracy.brighton-hove.gov.uk/documents/s205834/School\%20Admission\%20Arrangements\%202026-27.pdf}.}.
Of the school-level barriers our modelling is able to quantify ---
intake composition, prior attainment, workforce stability, segregation,
and absence --- none has a larger or better-evidenced effect on
disadvantaged attainment than absence. On the council's own framing, in
other words, the most direct route to \emph{``reducing barriers to
success''} for the city's disadvantaged pupils points squarely at
attendance.
...
The irony is considerable. Brighton and Hove ranks 15th best out of 151
...
One possibility is that absence is a fundamentally harder problem to
solve than admissions policies --- more diffuse in its causes, more
resistant to top-down policy intervention, and less amenable to the kind
of structural reform that admissions changes represent. But difficulty
of implementation should not be confused with importance of effect, and
the failure to even acknowledge absence as the primary drag on
attainment in the city represents a significant gap between the evidence
and the policy response.

\subsubsection{Absence is the mould, poverty is the
damp}\label{sec-mould-and-damp}

When an earlier version of this argument was presented locally, the
campaign group Class Divide offered a memorable reply: \emph{absence is
the mould, poverty is the damp}. On this view, treating absence directly
is futile, because the underlying poverty will simply keep bringing it
back.

The analogy captures something real, and it deserves a serious answer
rather than a dismissal. At pupil level the association between
disadvantage and absence is strong and well evidenced (DfE, 2025b), and
our own absence decomposition (Section~\ref{sec-decomposing-absence})
finds that a substantial part of school-level absence is inherited from
intake rather than generated by the school. Any attendance strategy that
ignores the material circumstances of families will disappoint, and we
...
difference in absence: regressing each authority's mean absence on its
mean FSM eligibility, across all 152 authorities in the latest year of
our data, yields an \(R^2\) of just 0.05. Poverty explains almost none
of why some areas have worse attendance than others.
...
\tightlist
\item
  Its FSM rate (28.8\%) is slightly \emph{below} the average across
  authorities (29.6\%). On a poverty-only account its absence should be
  a little below average too --- around 8.2\%.
\item
  It is 10.8\%. That is an excess of 2.6 percentage points over what its
  disadvantage profile predicts --- \textbf{the largest such excess of
  any authority in England}.
\item
  Among the 20 authorities whose FSM rates are within two percentage
  points of Brighton and Hove's, the city has the \emph{highest} absence
  of all, some 2.3 percentage points above their average.
\item
  And where absence fell in 94\% of local authorities between 2021--22
  and 2024--25, the city's did not move.
\end{itemize}

There is damp in many places. The mould is markedly worse here, and
unlike almost everywhere else it is not getting better.

The reasonable conclusion is not that poverty is irrelevant to absence
--- it plainly is not --- but that poverty cannot be the explanation for
Brighton and Hove's position, and that treating the city's attendance as
an inevitable consequence of its social composition is not supported by
the data. A substantial part of the city's absence sits above what its
intake predicts, and that is precisely the part local action can reach.

\subsubsection{How much can schools matter at
all?}\label{sec-how-much-schools-matter}

It is worth setting all of this against what school-effectiveness
research has consistently found, because the finding that redistribution
is a weak lever is neither novel nor contrarian. Ever since Coleman et
al. (1966), the share of variation in pupil outcomes attributable to
differences \emph{between} schools --- as opposed to differences in the
pupils they admit and the circumstances those pupils bring --- has been
estimated at something in the order of 8 to 15 per cent. The landmark
English study by Smith and Tomlinson (1989) settled on around a tenth,
and Teddlie and Reynolds (2000) confirm that broad range across
countries and phases. Our own decomposition of the between-school
component recovers the same order of magnitude.

This matters for what any admissions policy can be expected to achieve.
Moving pupils between schools operates on a comparatively small share of
the variance to begin with, and it does so without altering the intake
characteristics and social circumstances that account for most of the
rest. A redistribution policy works on the smaller term while leaving
the larger one untouched. That is not an argument for fatalism about
schools --- a tenth of the variance is still a great deal in a system of
three and a half thousand secondary schools --- but it is an argument
for realism about what changing \emph{which} school a child attends can
...
deserves reflection. The shift to a Leader and Cabinet system
concentrated decision-making power and enabled policy to be enacted
through a whipped voting system. The consultation that followed in late
2024 was compressed in its timeline and limited in its scope for
deliberative engagement. Without ease of data access, the time available
to assemble, analyse and present counter-evidence was inadequate for the
complexity of the issues at stake. Policy without genuine critical
scrutiny runs the risk of push-back and unintended consequences.
...
would predict. For disadvantaged pupils, the city ranks 15th; for
non-disadvantaged pupils, 4th; for all pupils, 5th in England.
...
A further finding from this work, developed through the absence
decomposition introduced in section 6, deserves explicit
acknowledgement. While absence consistently emerges as the most powerful
predictor of school-level attainment in our headline models, much of
that statistical force is borrowed from a structural component --- the
part of school-level absence that is inherited from intake, area
context, family circumstances, local health and the broader social
geography the school cannot itself choose. The school-controllable share
of absence --- what management, pastoral systems and attendance practice
can actually move --- is the smaller part. This does not diminish the
absence finding; it sharpens it. Closing a city like Brighton and Hove's
absence problem will require both school-level attention and
cross-departmental work --- public health, children's services, housing,
transport, area deprivation policy --- because much of what schools are
dealing with on attendance is not within their gift to fix alone. For
Brighton and Hove specifically, the city's extreme position on absence
is partly a structural inheritance and partly a school-management story,
and the policy response needs to engage both.

The same decomposition surfaces a fairer way of reporting school-level
performance. Reading a school's value-added alongside its residual
absence --- what we have called the joint-signal view --- separates
``this school adds points to attainment'' from ``this school's pupils
attend more than their intake predicts'' into two distinct signals that
the conventional single-residual league table folds together. Schools
delivering strong attainment outcomes through unusually good attendance
management are visible as a coherent management story; schools
delivering strong outcomes despite an attendance gap (BACA being the
city's clearest example) are visible as pedagogically impressive cases
with attendance still on the table as the next lever; and schools whose
mid-table raw attainment hides an attainment shortfall once intake is
properly accounted for (Patcham being the city's clearest example) are
visible as the most awkward cases for an LA to act on, where attendance
management is \emph{not} the available answer. Reporting both signals
side by side, with explicit uncertainty intervals, is a more honest
framing than the single number that conventional benchmarking produces
--- and would help align local policy conversation with what the data
...
Blagg K, Rosenboom V and Chingos MM (2018)
\emph{\href{https://www.urban.org/sites/default/files/publication/99027/time_to_school_and_student_outcomes_in_dc_1.pdf}{The
{Extra} {Mile}: {Time} to {School} and {Student} {Outcomes} in
...
Burgess S, Rawal S and Taylor ES (2022)
...
Chairassamee N, Chancharoenchai K and Saraithong W (2024) Getting there:
How commuting time and distance impact students' health. \emph{PLoS One}
19(12): e0314687. DOI:
\href{https://doi.org/10.1371/journal.pone.0314687}{10.1371/journal.pone.0314687}.
...
Coleman JS, Campbell EQ, Hobson CJ, et al. (1966) \emph{Equality of
{Educational} {Opportunity}}. Washington, DC: U.S. Department of Health,
...
Cordes SA, Rick C and Schwartz AE (2022) Do long bus rides drive down
academic outcomes? \emph{Educational Evaluation and Policy Analysis}
44(4): 689--716. DOI:
\href{https://doi.org/10.3102/01623737221092450}{10.3102/01623737221092450}.
...
Ding P, Li Y, Feng S, et al. (2023) Do long school commutes undermine
teenagers' wellbeing? {Evidence} from a nation-wide survey in {China}.
\emph{Journal of Transport \& Health} 30: 101605. DOI:
\href{https://doi.org/10.1016/j.jth.2023.101605}{10.1016/j.jth.2023.101605}.
...
Faulkner G, Stone M, Buliung R, et al. (2013) School travel and
children's physical activity: A cross-sectional study examining the
influence of distance. \emph{BMC Public Health} 13(1): 1166. DOI:
\href{https://doi.org/10.1186/1471-2458-13-1166}{10.1186/1471-2458-13-1166}.

\bibitem[\citeproctext]{ref-Fredriksen2004}
Fredriksen K, Rhodes J, Reddy R, et al. (2004) Sleepless in chicago:
Tracking the effects of adolescent sleep loss during the middle school
years. \emph{Child Development} 75(1): 84--95. DOI:
\url{https://doi.org/10.1111/j.1467-8624.2004.00655.x}.
...
Guan H, Xue J, Zhang Y, et al. (2025) Examining the relationship between
commuting time, academic achievement, and mental health in rural
{China}: A cross-sectional analysis. \emph{BMC Public Health} 25(1):
1616. DOI:
\href{https://doi.org/10.1186/s12889-025-22861-7}{10.1186/s12889-025-22861-7}.
...
Karan M, Rahal D, Almeida DM, et al. (2021) School commute time,
chronotype, and altered {HPA} axis functioning during adolescence.
\emph{Psychoneuroendocrinology} 133: 105371. DOI:
\href{https://doi.org/10.1016/j.psyneuen.2021.105371}{10.1016/j.psyneuen.2021.105371}.
...
Macleod S, Sharp C, Bernardinelli D, et al. (2015)
\emph{\href{https://www.nfer.ac.uk/publications/supporting-the-attainment-of-disadvantaged-pupils-articulating-success-and-good-practice/}{Supporting
the {Attainment} of {Disadvantaged} {Pupils}: {Articulating} {Success}
and {Good} {Practice}}}. London: Department for Education.
...
Nakajima S, Otsuka Y, Itani O, et al. (2024) Association between
commuting and mental health among {Japanese} adolescents.
\emph{Psychiatry and Clinical Neurosciences} 78(10): 588--594. DOI:
\href{https://doi.org/10.1111/pcn.13714}{10.1111/pcn.13714}.
...
Otsuka Y, Tokiya M, Saitoh I, et al. (2025) Factors associated with
school absenteeism due to difficulty awakening: A two-year prospective
cohort study of {Japanese} adolescents. \emph{Environmental Health and
Preventive Medicine} 30: 89--89. DOI:
\href{https://doi.org/10.1265/ehpm.25-00290}{10.1265/ehpm.25-00290}.

\bibitem[\citeproctext]{ref-Pereira2014}
Pereira EF, Moreno C and Louzada FM (2014) Increased commuting to school
time reduces sleep duration in adolescents. \emph{Chronobiology
International} 31(1): 87--94. DOI:
\href{https://doi.org/10.3109/07420528.2013.826238}{10.3109/07420528.2013.826238}.

\bibitem[\citeproctext]{ref-Pradhan2017}
Pradhan RK and Sinha N (2017) Impact of commuting distance and school
timing on sleep of school students. \emph{Sleep and Biological Rhythms}
15(2): 153--158. DOI:
\href{https://doi.org/10.1007/s41105-017-0091-0}{10.1007/s41105-017-0091-0}.
...
Shah KB (2023)
\emph{\emph{\href{https://knowledge.uchicago.edu/record/7216}{Traversing
the {City}: {Examining} the {Impacts} of {Extended} {Commute} on
{Student} {Wellbeing} \& {High} {School} {Experience} in {Chicago}}}}.
...
Smith DJ and Tomlinson S (1989) \emph{The {School} {Effect}: {A} {Study}
of {Multi}-{Racial} {Comprehensives}}. London: Policy Studies Institute.
...
Stein ML, Burdick-Will J and Grigg J (2021) A choice too far: Transit
difficulty and early high school transfer. \emph{Educational Researcher}
50(3): 137--144. DOI:
\href{https://doi.org/10.3102/0013189X20949504}{10.3102/0013189X20949504}.
...
Teddlie C and Reynolds D (2000) \emph{The {International} {Handbook} of
...
Voulgaris CT, Smart MJ and Taylor BD (2019) Tired of commuting?
{Relationships} among journeys to school, sleep, and exercise among
{American} teenagers. \emph{Journal of Planning Education and Research}
39(2): 142--154. DOI:
\href{https://doi.org/10.1177/0739456X17725148}{10.1177/0739456X17725148}.

\bibitem[\citeproctext]{ref-WassersteinLazar2016}
Wasserstein RL and Lazar NA (2016) The {ASA} statement on p-values:
Context, process, and purpose. \emph{The American Statistician} 70(2):
129--133. DOI:
...
Yeo SC, Jos AM, Erwin C, et al. (2019) Associations of sleep duration on
school nights with self-rated health, overweight, and depression
symptoms in adolescents: Problems and possible solutions. \emph{Sleep
Medicine} 60: 96--108. DOI:
\href{https://doi.org/10.1016/j.sleep.2018.10.041}{10.1016/j.sleep.2018.10.041}.

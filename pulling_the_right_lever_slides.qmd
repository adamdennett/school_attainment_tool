...
How much does the school really matter?
New, exploratory material — not in the accompanying paper
:::
...
## Where variation in Attainment 8 comes from: what a school inherits vs what it controls
...
Workforce — the part schools directly control — is a sliver (3%) next to the hand they are dealt (62%). Even the *persistent school effect* (16%) is an upper bound that still bundles catchment sorting.
:::
...
## What every pupil's Attainment 8 is really made of {.smaller}
...
.df <- bind_rows(.mk(.sed$decomp_all, 46, "Typical pupil"),
                 .mk(.sed$decomp_dis, .mdis, "Disadvantaged pupil"))
...
.df <- .df %>% mutate(grp = factor(grp, levels = rev(.lvl)),
                      who = factor(who, levels = c("Typical pupil", "Disadvantaged pupil")),
...
::: {style="font-size:0.5em; text-align:center; color:#555; margin-top:0.1em;"}
Bar height = each group's average score; each block = that factor's share of the variation in outcomes. The **pink** block — what leadership & teaching add — is real and visible for *every* pupil, and it is a **floor, not a ceiling**: a school like **BACA** adds ~1 SD *more* than the typical school for its disadvantaged pupils.
:::

## Why the pink block is not small — and why it is a floor {.smaller}

```{r school-impact-vals}
#| include: false
.srow <- .sed$decomp_dis[.sed$decomp_dis$Component == "School (persistent, unexplained)", ]
mean_dis  <- .sed$mean_att8_dis
sch_share <- .srow[["Share of total %"]]
sch_pts   <- round(mean_dis * .srow[["SD (log Att8)"]], 1)   # 1 SD, disadvantaged
sch_pts2  <- round(2 * sch_pts, 1)                            # 2 SD
# BACA's disadvantaged value-added = residual from the panel model
.baca <- bh_data %>%
  filter(grepl("Brighton Aldridge", SCHNAME),
         !is.na(ATT8SCR_FSM6CLA1A), ATT8SCR_FSM6CLA1A > 0) %>%
  mutate(r = ATT8SCR_FSM6CLA1A - exp(predicted_dis))
baca_4yr    <- round(mean(.baca$r), 1)
baca_latest <- round(.baca$r[.baca$year_label == max(.baca$year_label)][1], 1)
baca_sd     <- round(baca_4yr / sch_pts, 1)
```

The **`r sch_share`%** school share (disadvantaged pupils) is a *residual* — what a school adds **after intake, prior attainment, absence and neighbourhood are already accounted for**. It is worth **`r sch_pts` Attainment 8 points per standard deviation** between schools.

::: incremental
- **"1 SD above average"** = once everything measurable is stripped out, this school's disadvantaged pupils still do **~`r sch_pts` points better** than the typical school — about a **quarter-to-a-third of a grade in every subject**, or a **full grade in 2–3 subjects**
- **An excellent school** (2 SD, ~top 2.5%): **~`r sch_pts2` points**
- **Bigger than the lever the whole debate was about** (social-mixing ~1–2 points, and *not* for disadvantaged pupils); on par with an effective head [@Zuccollo2023] or strong teaching [@Slater2012]
- At the margin, **the difference between a grade 3 and a grade-4 pass** in English & Maths for many pupils
- **BACA *is* that school:** **+`r baca_4yr` Att8 points** for disadvantaged pupils over four years (**~`r baca_sd` SD** above the typical school), rising to **+`r baca_latest`** in 2024–25 — top-of-distribution *after* accounting for its tough intake
:::
...
quad_ylim_s <- c(min(qy_s[1], -8), max(qy_s[2], 8))
```
...
ggplot() +
  ...
  labs(x = "School-controllable absence (residual absence, pp)",
       ...
       title = "Brighton and Hove on the joint-signal plane") +
  theme_slide
```
...
## What we (now partly) know {.smaller}

The LEA random effect is **large and significant** — and the neighbourhood analysis has now answered *part* of what drives it.

::: incremental
- **Now measured:** ~`r round(100*(1 - bh_a$effect_census/bh_a$effect_base))`% of the city's all-pupil over-performance (~`r round(100*(1 - bh_d$effect_census/bh_d$effect_base))`% for disadvantaged) is its **educated adult population** — a graduate-heavy city where even disadvantaged pupils grow up among more educated neighbours. A **feature of place**, not a lever admissions can pull
- **But most survives:** after the strongest contextual control we can build, B&H is still **`r ordinal_suffix(bh_a$rank_census)`** (all pupils) / **`r ordinal_suffix(bh_d$rank_census)`** (disadvantaged) of 152 — the majority is something else
- Still unexplained: **leadership & governance**? Effective **LA support**? **Collaboration** between schools? A city-wide **culture** of engagement?
:::

:::: fragment
::: key-finding
So the "well-educated population" hunch is now partly quantified — but the larger, unexplained remainder *is* the well-functioning system. **Disrupting it before we understand what makes it work carries substantial downside risk.**
:::
::::

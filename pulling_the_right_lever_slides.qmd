...
::: incremental
- Outside of the school, does the neighbourhood environment matter?
- Census 2021 at **output-area** level: 
    - adult education (% level 4+), 
    - unemployment, 
    - economic inactivity 
    
— averaged over each school's **150 nearest OAs** (optimum number tested)
- Added to the full national model: prior attainment, disadvantage, absence, workforce, Ofsted **all already controlled**
- **It survives** — neighbourhood adult education statistically significant; marginal R² `r sprintf('%.2f', afr$r2$base_marginal)` → `r sprintf('%.2f', afr$r2$census_marginal)`
- Crucially: **the absence coefficient barely moves** at any k — the headline finding is not a neighbourhood artefact
:::

## What does a "150-nearest-neighbourhood" actually look like?

![](slides_assets/dorothy_stringer_150oa.png){fig-align="center" height="470"}

::: {style="font-size:0.55em; text-align:center; color:#555; margin-top:0.1em;"}
Pink diamond = **Dorothy Stringer School**; purple dots = its 150 nearest Output Area centroids — a catchment-scale ~1.5 km-radius neighbourhood.
:::

## Who was riding their neighbourhoods?

```{r nbhd-rerank-plot}
#| fig-height: 5.2
afr$la_cmp %>%
  ggplot(aes(mean_l4plus, effect_census - effect_base)) +
  geom_hline(yintercept = 0, linetype = "dashed", colour = "grey50") +
  geom_point(alpha = 0.55, size = 1.8, colour = "#49a0c4") +
  geom_point(data = afr$bh_all, colour = "#e16fca", size = 3.5) +
  geom_text_repel(
    data = afr$la_cmp %>%
      filter(rank(-abs(effect_census - effect_base)) <= 9 | grepl("Brighton", la_name)),
    aes(label = la_name), size = 3.2, max.overlaps = 20) +
  geom_smooth(method = "lm", se = FALSE, colour = "red", linewidth = 0.7) +
  labs(title = "Change in LA effect when neighbourhood context is measured",
       subtitle = paste0("r = ", sprintf('%.2f', afr$cor_shift_l4),
                         "  —  educated places lose part of their apparent over-performance"),
       x = "LA mean % adults with level 4+ qualifications (schools' 150 nearest OAs)",
       y = "Change in LA effect") +
  theme_slide
```

## The LA league table, re-ranked

::: incremental
- **Brighton & Hove (all pupils): `r ordinal_suffix(bh_a$rank_base)` → `r ordinal_suffix(bh_a$rank_census)`** of 152 — `r round(100*(1 - bh_a$effect_census/bh_a$effect_base))`% of the city's effect was its educated populace
- **Brighton & Hove (disadvantaged): `r ordinal_suffix(bh_d$rank_base)` → `r ordinal_suffix(bh_d$rank_census)`** — still top decile after the strongest contextual control we can build
- Inner London falls hard: `r paste(sprintf("%s %s→%s", fallers$la_name, fallers$rank_base, fallers$rank_census), collapse = "; ")`
- Genuine over-performers rise: `r paste(sprintf("%s %s→%s", risers$la_name, risers$rank_base, risers$rank_census), collapse = "; ")`
- Some of the residual "LA effect" in school performance (20%ish) is **neighbourhood demography** 
:::
...
## But the school effect is not small — what it means for a school {.smaller}

```{r school-impact-vals}
#| include: false
.sd_row  <- afr$decomp_all[afr$decomp_all$Component == "School (persistent, unexplained)", ]
sch_share <- .sd_row[["Share of total %"]]
sch_pts   <- round(46 * .sd_row[["SD (log Att8)"]], 1)   # 1 SD, all pupils
sch_pts2  <- round(2 * sch_pts, 1)                       # 2 SD
```

A **`r sch_share`%** variance share sounds abstract — but it converts into **`r sch_pts` Attainment 8 points per standard deviation** between schools (holding intake fixed).

::: incremental
- **A good school** (1 SD above average, ~top 16%) lifts *every* pupil by **~`r sch_pts` Att8 points** — about **a third of a grade in every subject**, or a **full grade up in 3–4 subjects**
- **An excellent school** (2 SD, ~top 2.5%): **~`r sch_pts2` points** — close to a **full grade across most GCSEs**
- **Bigger than the lever the whole debate was about** — social-mixing was worth ~1–2 points; this is 2–3× larger
- On par with the best-evidenced levers: effective headteacher ≈ **1 GCSE grade** [@Zuccollo2023]; good vs weak teaching ≈ **4.5 Att8 points** [@Slater2012]
- At the margin, **the difference between a grade 3 and a grade-4 pass** in English & Maths for many pupils
:::

:::: fragment
::: key-finding
And this is a *floor*, not a ceiling: it is the spread **between** schools with identical intakes — what leadership and teaching add. **The single biggest thing a school controls dwarfs anything admissions can move.**
:::
::::

...
4.  Re-calibrate how we view *'good schools'* - and convince you that most of what conflate with good schools, is out of their hands
...
::: key-finding
Bringing absence to the national average → predicted gains of **3--5 GCSE points** at the worst-affected schools. FSM reductions → closer to **1--2 points** (and not for disadvantaged students).
:::
...
- Is it a good school that simply has high levels of attainment?
- Is it a good school where pupils exceed the levels we would expect, given their circumstances?
...
# Neighbourhoods and the Limits of School Effects {.divider-slide background-color="#3A1857"}
...
## New: measuring the neighbourhood, not just the school

::: incremental
- Census 2021 at **output-area** level: adult education (% level 4+), unemployment, inactivity — averaged over each school's **150 nearest OAs**
- Added to the full national model: prior attainment, disadvantage, absence, workforce, Ofsted **all already controlled**
- **It survives** — neighbourhood adult education t ≈ 16; marginal R² `r sprintf('%.2f', afr$r2$base_marginal)` → `r sprintf('%.2f', afr$r2$census_marginal)`
- Bigger neighbourhoods fit better at KS4 — AIC optimises at **k = 150** (\~2.5 km), matching large secondary catchments (primaries prefer k = 10)
...
- The "LA effect" was never pure council performance — much of it is **measurable geography**
:::
...
```{r nbhd-decomp-all}
#| fig-height: 4.8
# Collapse the two fixed-effect rows into one combined bar for the big-picture
# view here; the split is revealed on the following slide.
afr$decomp_all %>%
  mutate(Component = ifelse(grepl("^Exogenous|^Endogenous", Component),
                            "Fixed effects (the hand each school is dealt)", Component)) %>%
  group_by(Component) %>%
  summarise(`Share of total %` = sum(`Share of total %`), .groups = "drop") %>%
  ...
  xlim(0, 80) +
  labs(title = "Variance decomposition: all pupils (national census model + school random intercept)",
       subtitle = "The persistent school component is the UPPER BOUND on what changing schools could achieve",

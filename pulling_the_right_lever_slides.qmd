...
.df <- bind_rows(.mk(.sed$decomp_all, 46, "All pupils"),
...
ggplot(.df, aes(who, pts, fill = grp)) +
  ...
  annotate("segment", x = 1.5, xend = 1.5, y = .mdis, yend = 46, colour = "grey55",
           arrow = arrow(ends = "both", length = unit(0.15, "cm")), linewidth = 0.4) +
  annotate("text", x = 1.5, y = (.mdis + 46)/2, label = sprintf("gap\n≈ %.0f pts", 46 - .mdis),
           size = 3.4, colour = "grey35", fontface = "italic") +
  scale_fill_manual(values = .pal, name = NULL, breaks = .lvl) +
  scale_y_continuous(limits = c(0, 52), expand = expansion(mult = c(0, 0.02))) +
...
::: {style="font-size:0.5em; text-align:center; color:#555; margin-top:0.1em;"}
Bar height = each group's average school-level score; each block = that factor's share of the variation in school outcomes. The **pink** block — what leadership & teaching add — is real and visible for **both groups**, and it is a **floor, not a ceiling**: a school like **BACA** adds \~1 SD *more* than the typical school for its disadvantaged pupils.
:::

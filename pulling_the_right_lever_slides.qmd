...
## First — today's response from Class Divide {.smaller}
...
- And we agree the city is **already among the most integrated in England** — a point even Professor Gorard, whose own work links segregation to the gap and who *supports* the reforms, affirms: our schools are more socially mixed than in most places.
...
- This is, in the end, partly a question of **values** as well as evidence - but this talk is concerned with the evidence.
:::
...
## Schools vs circumstances: a sixty-year argument {.smaller}
...
- **The statistical critique**: Goldstein & Spiegelhalter (1996), Leckie & Goldstein (2017) — school rankings carry so much uncertainty that league tables mislead; Gorard (2010) raises "serious doubts" about even the minority share
...
med_df <- imputed_full_data %>%
  filter(!is.na(ATT8SCR), ATT8SCR > 0, !is.na(PTFSM6CLA1A), PTFSM6CLA1A > 0,
         !is.na(PERCTOT), PERCTOT > 0, !is.na(PTPRIORLO))

m1_med <- lm(log(ATT8SCR) ~ log(PTFSM6CLA1A), data = med_df)
m3_med <- lm(log(ATT8SCR) ~ log(PTFSM6CLA1A) + log(PERCTOT) + PTPRIORLO, data = med_df)
b3 <- coef(m3_med)
...
## So what does a better school add? Closing the gap {.smaller}
...
ggplot(.bd, aes(who, pts, fill = part)) +
  ...
  annotate("text", x = 0.55, y = .typ + 1, label = "Typical (non-disadvantaged) pupil ≈ 46",
...
::: {style="font-size:0.5em; text-align:center; color:#555; margin-top:0.1em;"}
The school effect is **additive** — a good school adds its \~`r round(.sdsch,1)` points *to the pink block*, a very good school \~`r round(2*.sdsch,1)`. A genuinely good school lifts a disadvantaged pupil **most of the way to the typical pupil's average** — and the lever is about as strong here as for pupils overall. **BACA does exactly this: +`r baca_4yr` over four years.**
:::

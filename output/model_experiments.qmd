...
pupils with low KS2 prior attainment* (`PTPRIORLO`). The DfE publishes the mean
KS2 scaled score (`KS2ASS`) and the middle/high prior-attainment bands for only
three of our four years --- they are not released for 2024-25 --- so `PTPRIORLO`
is the only prior-attainment measure spanning the full panel. Here we refit on
the three-year sub-sample where the richer measure exists, adding mean KS2
score, and report how the key coefficients move.

...
1. **Wider neighbourhoods.** Secondary catchments are much larger than primary ones, so the context dataset (`R/13_census_context.R`) now provides k = 10, 25 and 50 nearest OAs (an OA averages ~125 households; k = 50 approximates a substantial slice of a town). The k-sensitivity comparison below lets the data choose.
...
fd %>%
  select(ATT8SCR, `k = 10 nearest OAs` = cens_l4plus_k10,
         `k = 50 nearest OAs` = cens_l4plus_k50) %>%
...
map_dfr(list("E baseline (joined sample)" = f_base,
  ...
  knitr::kable(caption = "Census context at KS4: contribution beyond the full Analysis E specification, by neighbourhood size")
```
...
f_best <- list("10" = f_k10, "25" = f_k25, "50" = f_k50)[[best_k_name]]
...
la_l4 <- fd %>%
  ...
  summarise(mean_l4plus = mean(cens_l4plus_k50, na.rm = TRUE), .groups = "drop")
...
ggplot(la_cmp, aes(mean_l4plus, effect_census - effect_base)) +
  ...
  labs(title = "Who was riding their neighbourhoods? LA effect change vs neighbourhood education",
       ...
       x = "LA mean % adults with level 4+ qualifications (schools' 50 nearest OAs)",
...
f_dis_census <- update(f_dis_base, . ~ . + z_l4plus_k50 + z_ltsick_k50 + z_unemp_k50)
...
- **k-sensitivity is informative, not nuisance**: if larger k fits better at KS4 than the k = 10 that suits primaries, that is the catchment-scale geography showing through in the data, consistent with the spatial programme's premise.

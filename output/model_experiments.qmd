...
base_rhs <- paste(
  "log(PERCTOT) + log(PNUMEAL) + PTPRIORLO + ADMPOL_PT +",
...
predict_fsm <- function(fit, label) {
  ...
  g <- tibble(
    ...
    PERCTOT = mn("PERCTOT"), PNUMEAL = mn("PNUMEAL"), PTPRIORLO = mn("PTPRIORLO"),
...
comp <- bind_rows(
  ...
  filter(Term %in% c("log(PTFSM6CLA1A)", "log(PERCTOT)", "PTPRIORLO")) %>%
...
rd_data <- imputed_full_data %>%
  filter(!is.na(ATT8SCR), ATT8SCR > 0,
         ...
         !is.na(PNUMEAL), PNUMEAL > 0,
         !is.na(PTPRIORLO), !is.na(gorard_segregation),
         !is.na(remained_in_the_same_school),
         !is.na(teachers_on_leadership_pay_range_percent),
         average_number_of_days_taken > 0,
         !is.na(OFSTEDRATING_1)) %>%
...
rd_rhs <- paste(
  "log(PTFSM6CLA1A) + log(PERCTOT) + log(PNUMEAL) + PTPRIORLO + ADMPOL_PT +",
  "gorard_segregation + remained_in_the_same_school +",
...
## Prior-attainment robustness (mean KS2 score)

The main models control for prior attainment using the *percentage of KS4
pupils with low KS2 prior attainment* (`PTPRIORLO`). It is important to be clear
that **no** prior-attainment measure is observed for 2024-25: the KS2 tests and
assessments of 2019-20 and 2020-21 were cancelled during the pandemic, so the
...
years. In the raw 2024-25 performance-table file, `KS2ASS`, `PTPRIORLO`,
`PTPRIORAV` and `PTPRIORHI` are all `NA` for every school. The values of
`PTPRIORLO` that appear in our 2024-25 panel rows are therefore **carry-forward
imputed** from each school's most recent observed year (or its multi-year mean),
and are flagged as such by `has_imputed_predictors`.

Here we refit on the three-year sub-sample where prior attainment is genuinely
observed, adding the mean KS2 scaled score alongside the low-prior-attainment
share, and report how the key coefficients move. This is the only sub-sample in
which the adequacy of the prior-attainment control can be assessed against real
data.
...
ks2_data <- rd_data %>%
  ...
  droplevels()
contrasts(ks2_data$OFSTEDRATING_1) <- contr.treatment(levels(ks2_data$OFSTEDRATING_1))

fit_ks2 <- function(extra) {
  rhs <- if (is.null(extra)) rd_rhs else paste(extra, "+", rd_rhs)
  lmer(as.formula(paste0("log(ATT8SCR) ~ ", rhs, " + ", rd_ranef)),
       data = ks2_data, REML = TRUE, control = ctrl_rd)
}

ks2_specs <- list("P0: published (PTPRIORLO only)" = NULL,
                  "P1: + mean KS2 score" = "ks2_c")

ks2_tab <- purrr::map_dfr(names(ks2_specs), function(nm) {
  m <- fit_ks2(ks2_specs[[nm]])
  s <- summary(m)$coefficients
  tibble(Specification = nm,
         log_FSM = s["log(PTFSM6CLA1A)", "Estimate"],
         log_Absence = s["log(PERCTOT)", "Estimate"],
         log_EAL = s["log(PNUMEAL)", "Estimate"],
         marginal_R2 = as.numeric(performance::r2(m)$R2_marginal))
})

knitr::kable(ks2_tab, digits = 4,
             caption = "All-pupil model on the 3-year sub-sample, with and without mean KS2 score. Adding a richer prior-attainment control leaves the absence coefficient essentially unchanged; the FSM coefficient attenuates.")
```
...
.p0 <- ks2_tab[grepl("P0", ks2_tab$Specification), ]
.p1 <- ks2_tab[grepl("P1", ks2_tab$Specification), ]
.abs_chg <- 100 * (.p1$log_Absence - .p0$log_Absence) / abs(.p0$log_Absence)
.fsm_chg <- 100 * (.p1$log_FSM - .p0$log_FSM) / abs(.p0$log_FSM)
cat(sprintf(
  "Adding mean KS2 score shifts the **absence** coefficient by only %.1f%% but the **FSM** coefficient by %.0f%%. The paper's central quantity --- absence --- is therefore robust to a substantially better prior-attainment control, while the (already weak) disadvantage-concentration coefficient attenuates further, reinforcing rather than weakening the argument.\n",
  .abs_chg, .fsm_chg))
```
...
.terms <- c("log(PTFSM6CLA1A)", "log(PERCTOT)", "log(PNUMEAL)", "PTPRIORLO")
comp <- tibble(
  Term = .terms,

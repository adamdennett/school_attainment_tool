...
fixed_var <- var(predict(f_school, re.form = NA), na.rm = TRUE)
...
decomp <- vc %>%
  transmute(Component = recode(grp,
    ...
    Variance = vcov) %>%
  bind_rows(tibble(Component = "Fixed effects (cohort, prior attainment, absence, workforce, neighbourhood)",
                   Variance = fixed_var)) %>%
  mutate(`Share of total %` = round(100 * Variance / total_var, 1),
         `SD (log Att8)` = round(sqrt(Variance), 4),
         `~Attainment 8 points (at mean 46)` = round(46 * sqrt(Variance), 1)) %>%
  arrange(desc(Variance))

knitr::kable(decomp %>% select(-Variance),
             caption = "Variance decomposition of log(Attainment 8), census model + school random intercept")
```
...
- Even taken at face value, compare its magnitude with the fixed-effects share: the hand a school is dealt — its cohort's composition, prior attainment, attendance and neighbourhood context — dwarfs the school's own persistent departure from expectation. Reallocating pupils between schools redistributes *which building children sit in* but changes neither their prior attainment, their home neighbourhoods, nor (directly) their attendance — the variables doing almost all the work. The plausible attainment gain from moving a child to a school one standard deviation "better" is a few Attainment 8 points at most, and that is before netting out the catchment-sorting component of the school effect.
...
decompose_model <- function(m, att8_mean) {
  ...
  fixed_var <- var(predict(m, re.form = NA), na.rm = TRUE)
  total_var <- fixed_var + sum(vc$vcov)
  vc %>%
    transmute(Component = recode(grp,
      "URN" = "School (persistent, unexplained)",
      ...
      Variance = vcov) %>%
    bind_rows(tibble(
      Component = "Fixed effects (cohort, prior attainment, absence, workforce, neighbourhood)",
      Variance = fixed_var)) %>%
    mutate(`Share of total %` = round(100 * Variance / total_var, 1),
           `SD (log Att8)` = round(sqrt(Variance), 4),
           `~Attainment 8 points` = round(att8_mean * sqrt(Variance), 1)) %>%
    arrange(desc(Variance))
}

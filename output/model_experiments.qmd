...
Holding `log(PTFSM6CLA1A)` as the deprivation measure, we swap the outcome variable across alternative Attainment 8 measures for disadvantaged pupils. Each captures a different component of the KS4 curriculum — English, Maths, EBacc subjects, and Open (non-EBacc) qualifications — allowing us to test whether the sign-flip is specific to the overall ATT8 aggregate or appears consistently across subject domains. For each, we report the coefficient on `log(PTFSM6CLA1A)`, its standard error, t-value, and sign.
...
cat(sprintf("Of the %d Attainment 8 component measures tested, **%d show a positive coefficient** on log(%% FSM) for disadvantaged pupils.\n\n", n_total, n_positive))

if (n_positive == n_total) {
  cat("The sign-flip is consistent across **every subject domain** — English, Maths, EBacc, and Open qualifications — not just the overall ATT8 aggregate. This makes it unlikely to be an artefact of how particular subjects are weighted in the Attainment 8 formula.\n\n")
} else if (n_positive >= n_total - 1) {
  cat("The sign-flip appears in almost all subject domains, suggesting it is a broad structural pattern rather than being driven by any single curriculum component.\n\n")
} else {
  cat("The sign-flip varies across subject domains, suggesting the relationship may be stronger in some curriculum areas than others.\n\n")
}

# Highlight any variation across components
eng_row <- y_results %>% filter(grepl("English", Y_Variable))
mat_row <- y_results %>% filter(grepl("Maths", Y_Variable))
if (nrow(eng_row) > 0 && nrow(mat_row) > 0 &&
    !is.na(eng_row$Coefficient) && !is.na(mat_row$Coefficient)) {
  cat(sprintf("Comparing the two core subjects: the coefficient is %+.5f for English and %+.5f for Maths. ",
      eng_row$Coefficient, mat_row$Coefficient))
  if (sign(eng_row$Coefficient) == sign(mat_row$Coefficient)) {
    cat("Both point in the same direction, reinforcing that the pattern is not subject-specific.\n\n")
  } else {
    cat("The difference in sign between these two core subjects warrants further investigation.\n\n")
  }
}
```

...
The FSM coefficient is the most diagnostic. If it inflates substantially as we move from M1 → M0 → M2, that tells us the absence variable was previously absorbing a chunk of the disadvantage signal, and our two-stage decomposition has redistributed it. If the coefficient on `log_expected_absence` in M2 is close to the M1 coefficient on `log(PERCTOT)`, the elasticity-on-absence reading we have been using is robust to the decomposition.
:::

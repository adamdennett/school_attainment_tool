...
# Load the pre-cleaning panel (with workforce + gorard but before ATT8 filtering)
# so that descriptive statistics reflect the full data landscape before modelling
panel_data <- readRDS(here::here("data", "panel_with_workforce.rds"))

# Add Gorard segregation + imputed predictors from the final panel
# (computed in 04_compute_derived.R but not in panel_with_workforce.rds)
panel_final <- readRDS(here::here("data", "panel_data.rds"))
gorard_cols <- c("gorard_segregation", "has_imputed_predictors")
gorard_data <- panel_final %>%
  select(URN, year_label, any_of(gorard_cols))
panel_data <- panel_data %>%
  left_join(gorard_data, by = c("URN", "year_label"))
...
model_panel <- panel_data %>%
  ...
  # Backfill any missing gor_name from other years for the same school
  group_by(URN) %>%
  mutate(gor_name = if (any(!is.na(gor_name))) coalesce(gor_name, na.omit(gor_name)[1]) else gor_name) %>%
  ungroup()
...
# Colour palette
year_cols <- c("2021-22" = "#2e6260", "2022-23" = "#abc766",
...
tibble::tribble(
  ...
  "KS4 Performance Tables", "DfE", "4 years (2021-22 to 2024-25)",
    "ATT8SCR, P8MEA, PTFSM6CLA1A, KS2ASS, PTPRIORLO, ADMPOL_PT, TOTPUPS",
...
model_vars_def <- c(
  ...
  "PTFSM6CLA1A", "PERCTOT", "PNUMEAL", "KS2ASS", "PTPRIORLO",
...
#| tbl-cap: "Panel dimensions by academic year (state-funded mainstream schools only)"
...
```{r}
#| label: tbl-dropped-summary
#| echo: false

dropped_types <- panel_data %>%
  filter(!MINORGROUP %in% c("Academy", "Maintained school")) %>%
  count(MINORGROUP, name = "Observations") %>%
  mutate(Schools = map_int(MINORGROUP, ~ n_distinct(panel_data$URN[panel_data$MINORGROUP == .x]))) %>%
  arrange(desc(Observations)) %>%
  rename(`School type` = MINORGROUP)

dropped_types %>%
  bind_rows(
    tibble(`School type` = "Total excluded",
           Observations = sum(dropped_types$Observations),
           Schools = NA_integer_)
  ) %>%
  kable(align = "lrr", format.args = list(big.mark = ",")) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE, font_size = 11) %>%
  row_spec(nrow(dropped_types) + 1, bold = TRUE)
```

This reduces the panel from `r format(nrow(panel_data), big.mark = ",")` to **`r format(nrow(model_panel), big.mark = ",")`** school-year observations (`r format(n_distinct(model_panel$URN), big.mark = ",")` unique schools across `r n_distinct(model_panel$LANAME)` LAs). These school types are excluded because they operate under different funding, governance, and accountability frameworks that make direct comparison with the mainstream state-funded sector inappropriate. Independent schools, in particular, do not participate in the same performance table reporting regime, and their inclusion would confound the relationship between deprivation-related predictors and attainment outcomes.
:::
...
Academies form the largest group in the analysis sample, followed by Maintained schools. Independent schools, colleges, and special schools have been excluded from all figures and tables in this report (see @tbl-school-type-filter for the full breakdown).
...
```{r tbl-outcome-stats-pooled}
#| label: tbl-outcome-stats-pooled
#| tbl-cap: "Descriptive statistics for Attainment 8 outcome variables (full panel, pooled across years)"
...
```{r tbl-predictor-stats-yearly}
#| label: tbl-predictor-stats-yearly
#| tbl-cap: "Descriptive statistics for predictor variables by academic year"
...
#| tbl-cap: "Descriptive statistics for predictor variables (full panel, pooled across years)"
...
all_model_vars <- c(
  "ATT8SCR", "ATT8SCR_FSM6CLA1A", "ATT8SCR_NFSM6CLA1A",
  "PTFSM6CLA1A", "PERCTOT", "PPERSABS10", "PNUMEAL", "KS2ASS", "PTPRIORLO",
...
::: callout-note
## Teacher sickness absence: a notable gap in 2021--22 and 2022--23
...
Five predictor variables are unavailable for the 2024--25 academic year:

-   **KS2ASS** — the cohort mean KS2 scaled score, which is the prior-attainment control used in the models (entered centred at 100 as `ks2_c`). No prior-attainment measure exists at all for this year: the KS2 assessments of 2019--20 and 2020--21 were cancelled during the pandemic, so the cohort reaching the end of KS4 in 2024--25 has no KS2 results, and the DfE publishes neither prior-attainment measures nor Progress 8 for it
-   **PTPRIORLO** — the share of pupils with low KS2 prior attainment, unavailable for the same reason. It is retained in the panel for the specification comparison reported in the model experiments, but is no longer the prior-attainment control
...
impute_vars <- c("KS2ASS", "PTPRIORLO", "remained_in_the_same_school",
                 "teachers_on_leadership_pay_range_percent",
                 "average_number_of_days_taken")
...
A key question for the carry-forward strategy is whether imputed variables are sufficiently stable from one year to the next at the school level. @fig-ks2ass-scatter addresses this for KS2ASS (the cohort mean KS2 scaled score, the prior-attainment control used in the models) by plotting each school's value in one year against its value in the next. If the variable were perfectly stable, all points would fall on the 45-degree line.

```{r fig-ks2ass-scatter}
#| label: fig-ks2ass-scatter
#| fig-cap: "Year-on-year stability of KS2 prior attainment (mean scaled score) at the school level"
#| fig-height: 5
#| fig-width: 10

# Build wide data: one row per school with each year's KS2ASS
prior_wide <- model_panel %>%
  filter(!is.na(KS2ASS)) %>%
  select(URN, year_label, KS2ASS) %>%
  pivot_wider(names_from = year_label, values_from = KS2ASS,
              names_prefix = "y_")
...
p_scatter1 <- ggplot(pair1, aes(x = `y_2021-22`, y = `y_2022-23`)) +
  ...
  labs(x = "mean KS2 scaled score (2021-22)",
       y = "mean KS2 scaled score (2022-23)",
...
p_scatter2 <- ggplot(pair2, aes(x = `y_2022-23`, y = `y_2023-24`)) +
  ...
  annotate("text", x = Inf, y = -Inf,
           ...
           hjust = 1.1, vjust = -0.5, size = 3.5, colour = "grey30") +
  labs(x = "mean KS2 scaled score (2022-23)",
       y = "mean KS2 scaled score (2023-24)",
...
p_prior_scatter <- p_scatter1 + p_scatter2 +
  plot_annotation(
    title = "Year-on-year stability of KS2ASS at the school level",
    subtitle = "Dashed line = perfect carry-forward (y = x)",
    theme = theme(plot.title = element_text(face = "bold", size = 12),
                  plot.subtitle = element_text(colour = "grey40", size = 10))
  )

save_fig(p_prior_scatter, "ks2ass_year_scatter", width = 10, height = 5)
p_prior_scatter
```

Both panels show a tight clustering of schools along the 45-degree line, with high year-on-year correlations. This confirms that a school's mean KS2 intake score is a stable, slow-moving characteristic. The degree of scatter is modest and symmetric around the diagonal, indicating no systematic drift. This pattern provides strong support for the carry-forward imputation of KS2ASS into 2024--25: if a school's value barely changes from 2021--22 to 2022--23 or from 2022--23 to 2023--24, it is reasonable to assume a similar level of stability into the following year.
...
cor_vars <- c("ATT8SCR", "PTFSM6CLA1A", "PERCTOT", "PNUMEAL",
              "KS2ASS", "PTPRIORLO", "gorard_segregation",
              "remained_in_the_same_school",
              "teachers_on_leadership_pay_range_percent",
              "average_number_of_days_taken")

...
Four nested models are fitted to the same data (Analysis E, all 4 years):
...
3. **Model 3** — all nine fixed-effect predictors (no random effects)
4. **Model 4** — the full multilevel model with random intercepts for year, Ofsted rating, and local authority nested within region
...
m3 <- lm(log(ATT8SCR) ~ log(PTFSM6CLA1A) + log(PERCTOT) + log(PNUMEAL) +
...
m4 <- lmer(log(ATT8SCR) ~ log(PTFSM6CLA1A) + log(PERCTOT) + log(PNUMEAL) +
             PTPRIORLO + ADMPOL_PT + gorard_segregation +
             remained_in_the_same_school +
             teachers_on_leadership_pay_range_percent +
             log(average_number_of_days_taken) +
...
extract_coefs <- function(model, is_lmer = FALSE) {
  ...
  # Column names differ between lm and lmer
  ...
  df %>%
    transmute(
      ...
      est_display = paste0(format(round(estimate, 4), nsmall = 4), stars),
...
c2 <- extract_coefs(m2)
c3 <- extract_coefs(m3)
c4 <- extract_coefs(m4, is_lmer = TRUE)

# ---- Build unified term list (in model order) ----
all_terms <- unique(c(c1$term, c2$term, c3$term, c4$term))
# Keep intercept first, then order by appearance in full model
term_order <- c4$term
all_terms <- c(term_order, setdiff(all_terms, term_order))
...
r2_m4 <- performance::r2(m4)
r2m_m4 <- round(r2_m4$R2_marginal, 4)
r2c_m4 <- round(r2_m4$R2_conditional, 4)
...
footer_rows <- tibble(
  ...
  t3   = c("", "", ""),
  est4 = c(as.character(r2m_m4), as.character(r2c_m4), format(n_obs, big.mark = ",")),
  t4   = c("(marginal)", "(fixed + random)", "")
)

full_tbl <- bind_rows(step_tbl, footer_rows)
...
kbl(full_tbl, format = "html", align = "lrlrlrlrl", escape = FALSE) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed"),
                full_width = FALSE, font_size = 11) %>%
  add_header_above(c(" " = 1,
                      "Model 1: FSM only" = 2,
                      "Model 2: + Absence" = 2,
                      "Model 3: All fixed effects" = 2,
                      "Model 4: Full multilevel" = 2)) %>%
  row_spec(nrow(step_tbl) + 1, bold = TRUE) %>%
  row_spec(nrow(step_tbl) + 2, bold = TRUE) %>%
  row_spec(nrow(step_tbl) + 3, bold = TRUE) %>%
  footnote(general = "Significance: *** p < 0.001, ** p < 0.01, * p < 0.05. Models 1–3 are OLS; Model 4 is a multilevel model (lmer) with random intercepts.",
           general_title = "")
```
...
fsm_m1 <- round(c1$estimate[c1$term == "log(PTFSM6CLA1A)"], 4)
fsm_m2 <- round(c2$estimate[c2$term == "log(PTFSM6CLA1A)"], 4)
fsm_m3 <- round(c3$estimate[c3$term == "log(PTFSM6CLA1A)"], 4)
fsm_m4 <- round(c4$estimate[c4$term == "log(PTFSM6CLA1A)"], 4)
abs_m2 <- round(c2$estimate[c2$term == "log(PERCTOT)"], 4)
abs_m3 <- round(c3$estimate[c3$term == "log(PERCTOT)"], 4)
abs_m4 <- round(c4$estimate[c4$term == "log(PERCTOT)"], 4)
```
...
**Model 2 → Model 3 (adding all remaining fixed effects):** The FSM coefficient changes again to `r fsm_m3` as prior attainment, admissions policy, EAL, segregation, and workforce variables enter the model. Prior attainment is a particularly important mediator — schools with high FSM rates also have more pupils with low prior attainment, and separating these effects sharpens the estimate of each. The absence coefficient also shifts from `r abs_m2` to `r abs_m3`. R² rises to `r r2_m3`, showing that the full set of school-level characteristics explains substantially more variation than disadvantage and absence alone.

**Model 3 → Model 4 (adding random effects):** The fixed-effect coefficients (R²~m~ = `r r2m_m4`) are similar to Model 3, but the conditional R² (R²~c~ = `r r2c_m4`) shows a substantial jump. The difference between R²~m~ and R²~c~ represents variance explained by the **grouping structure** — knowing which Ofsted category, region, and local authority a school belongs to adds considerable explanatory power beyond the nine measured predictors. This suggests that unobserved LA-level and regional factors (funding levels, local labour markets, housing costs, historical policy choices) contribute meaningfully to attainment differences.

...
# Measuring Model Fit: Marginal and Conditional R²
...
## Why not just use the ordinary R²?
...
## The Nakagawa & Schielzeth decomposition

[Nakagawa and Schielzeth (2013)](https://doi.org/10.1111/j.2041-210x.2012.00261.x), extended by [Nakagawa, Johnson and Schielzeth (2017)](https://doi.org/10.1098/rsif.2017.0213), provide a variance-partitioning definition of $R^2$ that does generalise to mixed models. The total variance is decomposed as:
...
$$
R^2_m \;=\; \frac{\sigma^2_f}{\sigma^2_f + \sum_k \sigma^2_{u_k} + \sigma^2_{\varepsilon}}
\qquad\qquad
R^2_c \;=\; \frac{\sigma^2_f + \sum_k \sigma^2_{u_k}}{\sigma^2_f + \sum_k \sigma^2_{u_k} + \sigma^2_{\varepsilon}}
$$
...
### Isn't this just the squared correlation between observed and fitted values?
...
## How this relates to the fit measures usually reported for LMMs
...
School Workforce Census variables (`remained_in_the_same_school`, `teachers_on_leadership_pay_range_percent`, `average_number_of_days_taken`) and KS2 prior attainment (`KS2ASS`, entered in the models as `ks2_c`) are not available for 2024-25; the relevant cohort's primary assessments were cancelled during the pandemic. To enable cross-year comparison including the latest year, a **core specification** is fitted using only the 5 predictors available across all four years:
...
prior_m3 <- round(c3$estimate[c3$term == "ks2_c"], 3)
```
...
fit_stepwise <- function(data, outcome_var, caption_label) {
  ...
  m3 <- lm(as.formula(paste0(fml_base, "log(PTFSM6CLA1A) + log(PERCTOT) + ks2_c")), data = d)
...
d_prior_m3 <- round(sw_dis$coefs$c3$estimate[sw_dis$coefs$c3$term == "ks2_c"], 3)
```
...
n_prior_m3 <- round(sw_non$coefs$c3$estimate[sw_non$coefs$c3$term == "ks2_c"], 3)
```
...
comp_prior_dis <- round(dis_c5$estimate[dis_c5$term == "ks2_c"], 3)
comp_prior_non <- round(non_c5$estimate[non_c5$term == "ks2_c"], 3)
...
model_vars <- c(
  ...
  "PTFSM6CLA1A", "PERCTOT", "PNUMEAL", "KS2ASS",
  "ADMPOL_PT", "gorard_segregation",
  "remained_in_the_same_school",
  "teachers_on_leadership_pay_range_percent",
...
tibble(
  ...
  Source = case_when(
    model_vars %in% c("ATT8SCR", "ATT8SCR_FSM6CLA1A", "ATT8SCR_NFSM6CLA1A",
                       "PTFSM6CLA1A", "KS2ASS", "ADMPOL_PT") ~ "KS4 Performance Tables",

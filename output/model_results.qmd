...
**Model 1 → Model 2 (adding absence):** For disadvantaged pupils, the FSM coefficient starts at `r d_fsm_m1` and shifts to `r d_fsm_m2` when absence enters. As with all pupils, the direction of confounding is clear — absence partly mediates the FSM–attainment pathway. R² increases from `r sw_dis$r2$r2_1` to `r sw_dis$r2$r2_2`.
...
The admissions policy dummies are excluded because standardising a binary (0/1) indicator is less informative — a "one SD change" in a dummy variable has no natural interpretation.

...
The interactive scatter plot below allows exploration of the bivariate relationships between each Attainment 8 outcome and the predictor variables as they enter the multilevel model (log-transformed where applicable). Use the dropdowns to select the outcome variable (y-axis), predictor (x-axis), and filter by academic year.
...
# --- Prepare data with log-transformed predictors matching the model spec ---
bivar_df <- model_data %>%
  mutate(
    `log(% Disadvantaged)` = log(PTFSM6CLA1A),
    `log(% Absence)` = log(PERCTOT),
    `log(% EAL)` = log(PNUMEAL),
    `% Low Prior Attainment (KS2)` = PTPRIORLO,
    `Gorard Segregation Index` = gorard_segregation,
    `log(Teachers Remaining FTE)` = log(remained_in_the_same_school),
    `% Teachers on Leadership Pay` = teachers_on_leadership_pay_range_percent,
    `log(Avg. Teacher Sickness Days)` = log(average_number_of_days_taken)
  ) %>%
  filter(!is.na(ATT8SCR))

# Hover label
if ("SCHNAME" %in% names(bivar_df) && "LANAME" %in% names(bivar_df)) {
  bivar_df$hover_label <- paste0(bivar_df$SCHNAME, " (", bivar_df$LANAME, ")")
} else {
  bivar_df$hover_label <- paste0("URN ", bivar_df$URN)
}

# Predictor columns (as they appear in the model formula)
pred_cols <- c(
  "log(% Disadvantaged)",
  "log(% Absence)",
  "log(% EAL)",
  ...
  "log(Teachers Remaining FTE)",
  "% Teachers on Leadership Pay",
  "log(Avg. Teacher Sickness Days)"
)
pred_cols <- pred_cols[vapply(pred_cols, function(pc) any(!is.na(bivar_df[[pc]])), logical(1))]

# Outcome columns
outcome_cols <- c(
  ATT8SCR             = "Attainment 8 — All Pupils",
  ATT8SCR_FSM6CLA1A   = "Attainment 8 — Disadvantaged",
  ATT8SCR_NFSM6CLA1A  = "Attainment 8 — Non-Disadvantaged"
)
...
# --- Build base plotly figure (one trace per year, no updatemenus) ---
fig <- plot_ly()
for (i in seq_along(years)) {
  df_i <- year_dfs[[i]]
  fig <- fig %>%
    add_markers(
      x = df_i[["log(% Disadvantaged)"]],
      y = df_i[["ATT8SCR"]],
...
# Regression line trace (initially hidden — JS computes OLS and shows on load)
fig <- fig %>%
  add_lines(
    x = c(0, 1), y = c(0, 1),
    name = "OLS fit",
...
fig <- fig %>%
  layout(
    xaxis  = list(title = pred_cols[1]),
    yaxis  = list(title = unname(outcome_cols[1])),
    legend = list(orientation = "h", x = 0.2, y = -0.12),
    margin = list(t = 30, l = 80)
  )

fig$elementId <- "bivar-scatter"

# --- Pack column data into JSON for the JS updater ---
js_data <- list()
for (yr in years) {
  yr_list <- list()
  df <- year_dfs[[yr]]
  for (pc in pred_cols)          yr_list[[pc]] <- round(df[[pc]], 5)
  for (oc in names(outcome_cols)) yr_list[[oc]] <- round(df[[oc]], 4)
  js_data[[yr]] <- yr_list
}

# --- HTML dropdown controls (rendered above the plot) ---
select_style <- paste0(
...
controls <- tags$div(
  ...
  tags$div(
    tags$label("Outcome (y-axis):", `for` = "bivar-outcome", style = label_style),
    tags$select(
      id = "bivar-outcome", style = paste0(select_style, "min-width:230px;"),
      lapply(seq_along(outcome_cols), function(k)
        tags$option(value = names(outcome_cols)[k], unname(outcome_cols[k])))
    )
  ),
  tags$div(
    tags$label("Predictor (x-axis):", `for` = "bivar-predictor", style = label_style),
    tags$select(
      id = "bivar-predictor", style = paste0(select_style, "min-width:270px;"),
      lapply(pred_cols, function(pc) tags$option(value = pc, pc))
    )
  ),
  tags$div(
    tags$label("Year:", `for` = "bivar-year", style = label_style),
    tags$select(
      id = "bivar-year", style = paste0(select_style, "min-width:140px;"),
...
(function() {
  ...
  var YEARS = %s;
  var N     = YEARS.length;
  var LINE  = N;  /* trace index of the OLS regression line */

  /* ---- Ordinary Least Squares from two arrays ---- */
  ...
  /* ---- Main update ---- */
  function update() {
    var gd = document.getElementById("bivar-scatter");
    if (!gd || !gd._fullLayout) return;

    var outVal  = document.getElementById("bivar-outcome").value;
    var predVal = document.getElementById("bivar-predictor").value;
    var yrVal   = document.getElementById("bivar-year").value;

    var xArr = [], yArr = [], visArr = [];
    var allX = [], allY = [];
    for (var i = 0; i < N; i++) {
      var yr = YEARS[i];
      var xd = DATA[yr][predVal] || [];
      var yd = DATA[yr][outVal]  || [];
    ...
    /* update scatter traces (indices 0..N-1) */
    var idx = []; for (var i = 0; i < N; i++) idx.push(i);
    Plotly.restyle(gd, { x: xArr, y: yArr, visible: visArr }, idx);

    /* compute OLS and update regression line + annotation */
    var outLabel = document.getElementById("bivar-outcome")
  ...
  ["bivar-outcome","bivar-predictor","bivar-year"].forEach(function(id) {
    document.getElementById(id).addEventListener("change", update);
  });

  /* poll until the plotly widget is initialised, then sync once */
...
# --- Render: controls, then plot widget, then JS wiring ---
...
model_data %>%
  ...
  geom_point(data = raw_means_ofsted, aes(y = raw_mean),
             shape = 4, size = 3, colour = "#d62728", stroke = 1.2) +
  geom_point(data = raw_means_ofsted, aes(y = shrunken_mean),
             shape = 18, size = 4, colour = "#1565c0") +
  ...
  labs(title = "ATT8 Distribution by Ofsted Rating (Null Model Structure)",
       subtitle = "Dashed = grand mean | Blue diamond = shrunken mean (BLUP) | Red cross = raw mean",
...
ggplot(re_la, aes(x = reorder(group, `(Intercept)`), y = `(Intercept)`)) +
  ...
  scale_colour_manual(values = c("Other" = "steelblue",
...
#| fig-cap: "Caterpillar plot of LA random intercepts compared across model specifications (All Pupils). Brighton and Hove highlighted in orange."
#| fig-height: 12
#| fig-width: 10
...
The scatter plots below show observed Attainment 8 scores against imputed-full-model fitted values (Analysis E) for each of the four academic years. Schools in **Brighton and Hove** are highlighted in orange. A linear regression line (red) with R² is overlaid on each panel; the dashed grey line marks perfect prediction.

## All Pupils

```{r fig-obs-vs-pred-all}
#| label: fig-obs-vs-pred-all
#| fig-cap: "Observed vs predicted Attainment 8 — All Pupils (imputed full model, by year). Brighton and Hove schools in orange."
#| fig-height: 8
#| fig-width: 10
...
#| fig-cap: "Observed vs predicted Attainment 8 — Disadvantaged Pupils (imputed full model, by year). Brighton and Hove schools in orange."
#| fig-height: 8
#| fig-width: 10
...
#| fig-cap: "Observed vs predicted Attainment 8 — Non-Disadvantaged Pupils (imputed full model, by year). Brighton and Hove schools in orange."
#| fig-height: 8
#| fig-width: 10

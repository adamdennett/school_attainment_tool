...
subtitle: "School Attainment, Open Data Analytics and Local Education Policy in England — Lessons from Brighton and Hove (v2.0)"
author:
  - name: Adam Dennett
...
format:
  revealjs:
    ...
    output-file: "pulling_the_right_lever_short_slides_v2.html"
output-dir: docs
...
- Over the last 4-years, after accounting for structural factors, Brighton and Hove ranks (out of `r rk_dis$n` LEAs):
...
- A two-stage decomposition[^twostage-short] separates them, giving each school a **value-added** *and* a **residual absence** indicator
...
[^twostage-short]: Full analysis: <https://adamdennett.github.io/school_attainment_tool/model_experiments.html#sec-two-stage-absence>


## Brighton and Hove on the joint-signal plane

```{r quadrant-fits-short}
#| label: quadrant-fits-short
#| include: false
...
```{r fig-bh-quadrant-short}
#| label: fig-bh-quadrant-short
#| fig-height: 5
...
# Neighbourhoods and the Limits of School Effects {background-color="#3A1857" .divider-slide}
...
## New: the neighbourhood enters the model {.smaller}

::: {.incremental}
- Census 2021 adult education, unemployment and inactivity for every school's **50 nearest output areas** — added to the full model (prior attainment, disadvantage, absence, workforce all controlled)
- **It survives** (t ≈ 16) — and re-ranks the LA league table: educated places lose part of their apparent over-performance (r = `r sprintf('%.2f', afr$cor_shift_l4)` with neighbourhood education)
- **Brighton & Hove: `r ordinal_suffix(bh_a$rank_base)` → `r ordinal_suffix(bh_a$rank_census)`** (all pupils), **`r ordinal_suffix(bh_d$rank_base)` → `r ordinal_suffix(bh_d$rank_census)`** (disadvantaged) — about a `r ifelse(round(100*(1 - bh_a$effect_census/bh_a$effect_base)) >= 30, "third", "fifth")` of the city's effect is its educated populace; **still top decile** after the strongest contextual control we can build
- The **absence coefficient barely moves** — the headline finding is not a neighbourhood artefact
:::
...
afr$decomp_all %>%
  ...
  xlim(0, 75) +
  labs(title = "Variance decomposition: all pupils (census model + school random intercept)",
...
## The same picture for disadvantaged pupils {.smaller}
...
::: {.incremental}
- Persistent school effect ≈ **`r afr$decomp_dis[["Share of total %"]][afr$decomp_dis$Component == "School (persistent, unexplained)"]`%** (≈ `r afr$decomp_dis[["~Attainment 8 points"]][afr$decomp_dis$Component == "School (persistent, unexplained)"]` Att8 points per SD, *upper bound*); the hand the school is dealt ≈ **`r afr$decomp_dis[["Share of total %"]][grepl("Fixed", afr$decomp_dis$Component)]`%**
